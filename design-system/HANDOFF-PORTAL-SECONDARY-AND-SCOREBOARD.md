...
> **STATUS 2026-05-08 efter granskning:**  
> **Del A (Portal cards):** Designbeslut tagna, klar för implementation. Se BESLUT-sektion nedan.  
> **Del B (Scoreboard):** PAUS — väntar Design-normalisering av match-yta-grammatik (industrial-LED vs copper-warm-konflikt). Återupptas när normaliserat förslag levereras.
>
> **Code-instruktion för Del A:** `docs/CODE_INSTRUCTION_PORTAL_REDESIGN_2026-05-08.md`
...
- `docs/mockups/2026-05-08_design_portal-secondary-cards.html` — tre Portal-secondary-cards
- `docs/mockups/2026-05-08_design_scoreboard-stalvallen.html` — matchvy-scoreboard (mekanisk + digital lager)

---

## DEL A · Portal secondary cards

### BESLUT 2026-05-08 — vad som gäller för implementation

**Framing:** Detta är **redesign av tre existerande komponenter**, inte ny implementation. Komponenterna finns redan i `src/presentation/components/portal/secondary/` (commits `ad43cce`, `3b06ce6`, `35d6a44`). Code ska börja med diff mot existerande, inte rewrite.

**Vad behålls från existerande kod:**
- `WeeklyDecisionSecondary` — `CATEGORY_META`-systemet med 4 kategorier (player/supporter/training/community) och dess label-varianter
- `WeeklyDecisionSecondary` — resolution-mönstret (setTimeout 1500ms + ✓-checkmark + effekt-text)
- `BoardObjectivesSecondary` — `formatOwnerInitial()` med fullnamn ("P. Andersson", inte "PELLE")
- `BoardObjectivesSecondary` — domänmodellen (`measureFn`, `currentValue`, `targetValue`). Mockens `display.kind` mappas via funktion i komponenten, ej domain-migration
- `ActiveArcsSecondary` — `ARC_ICON[arc.type]` från `activeArcStrings`, ej hardcoded 4 emojier
- Antal rader visade: `slice(0, 2)` på alla tre tills playoff-test ger anledning att växla till 3

**Vad anammas från mock:**
- Stripe-systemet (2 px `--accent` för innehåll, 3 px `--warm` för relations/persona)
- Knapp-stil i WeeklyDecision: transparent + accent border (likvärdiga, ej solid+transparent)
- WeeklyDecision frågans typografi: Georgia italic 14 px line-height 1.5 (ej sans-semibold)
- BoardObjective status-presentation: bara icon (📌⚠️❌✅), ej text-label "Aktivt"/"I fara"
- Phase-dots-styling och alla layout-detaljer i mocken

**Öppna frågor markerade som "sannolikt mock-layout" i granskningen:**
- Sektionsrubriker `⊩ Veckans fråga ⊩` — ignorera, mock-layout för kommunikation
- Mock-fil `_base.css`-länken — trasig relativ sökväg, ej Code's problem

Full granskning: `docs/DESIGN_REVIEW_2026-05-08.md`
...
> **PAUS 2026-05-08** — Match-vy-grammatik kräver normalisering innan implementation.  
> Granskningen identifierade konflikt mellan mockens industrial-LED-grammatik och befintlig copper-warm-palett i resten av match-vyn (`MatchHeader`, `CommentaryFeed`, `MatchLiveScreen`-bg, halftime-modal). Design tar fram normaliserat förslag som täcker scoreboard + drift-matchsekvenser tillsammans. Återupptas när leverans landar.
>
> Specen nedan behålls som referens för teknisk struktur (tre moduler, tidslinje-koncept, operatör-textremsa). Mönstret behöver en visuell grammatik som spelar med övrig match-yta innan Code rör kod.
