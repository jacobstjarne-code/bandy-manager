...
**GRIND 3 — commit-hash-existens.** En grind som greppar `docs/archive/historiska-statuskallor/SLUTTEST_KO.md` + `docs/archive/historiska-statuskallor/BACKLOG.md` efter `KLAR <hash>` / `(\`[0-9a-f]{7,}\`)`-mönster och kör `git cat-file -e <hash>` på var och en. Saknad/feltypad hash = violation. Fångar fabricerade och feltypade referenser.
