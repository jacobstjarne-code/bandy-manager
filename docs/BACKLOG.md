...
| # | Idé | Plats | Estimat |
|---|---|---|---|
| C-SD1 | **Säsongsslutets koreografi — scener i fel ordning/dubblerade.** Halvvägs-portal + halvvägs-scen säger samma sak i rad. Sommaren-scenen kommer efter match 22 MEN före slutspelet (sekvens-bugg: "i oktober är det igång igen" medan kvartsfinal återstår). Grundserien-avklarad + säsongen-klar + sommaren = tre avsluts-skärmar i konstig ordning. Ingen scen äger SEKVENSEN. Sommaren måste gated på slutspel klart. **Släkt med C-SP1** (samma "övergång vet inte tillståndet"-klass). | scen-triggers / SeasonPhase-flöde | **VERIFIERAT 2026-05-25 (kolläst):** `seasonEndPhase.ts` har redan 6 faser utan `'summer'`; inga `inSummerScene`/`seasonDoneAck` på SaveGame. Sommaren (koreografi-handoffens 7:e fas) byggdes aldrig — det finns INGEN sommar-död-kod att ta bort. `regular_done`-anslag SKRIVET (`leagueAnslag.ts`). KVAR för Code (litet): verifiera/wire:a att `regular_done`-anslaget fyrar EN gång vid grundserie→slutspel (spegla `season_done`-skärmen, ingen ny tier). Framtida ej akut: manuell sommar-fas som egen feature om vi vill ha den — ritad i `HANDOFF-C-SD1-KOREOGRAFI-2026-05-23`. |
...
| # | Idé | Plats | Estimat |
|---|---|---|---|
| C-U1 | **Utvisning→avstängning-legibilitet (orsakskrok).** Avstängningen visas i inkorgen och blickfångs-bågen (efter utvisningen) löper, men ORSAKEN syns aldrig: vilken match, vilket matchstraff. Spelarens fråga: var och hur blev han avstängd? Fix: avstängningen bär sin orsak (Avstängd nästa match efter matchstraff mot X, omg N) + bågen refererar utlösande match. Samma orsakskrok som C-FT1/C-SY1. | suspension/utvisning-system + inbox + arc | Medel. Design+Code. Spåra avstängningssystemet först — lagras orsaken? |
