...
Äldre poster ligger i samma ordning som i MASTER_OPPET.md vid arkiveringstillfället; nya stängningar läggs överst i registret.

| id | status | commit | källpekare |
|---|---|---|---|
| bild-slots-tre-scener | klar | denna commit | Tre godkända slutillustrationer är produktbehandlade som scenbilder — ingen friläggning, alfa- eller cirkelbeskärning — genom den etablerade sharp-pipelinen (WebP q80, maxbredd 1170 px). `burnout-ceiling.webp` är 1170×2096/52 KiB, `klack-tifo.webp` 1170×653/32 KiB och `klack-konflikt.webp` 1170×653/40 KiB; de högupplösta JPEG-originalen är bevarade i momentbildsarkivet. BurnoutMark bär nu den vertikala bilden i en 238 px `band` med text i en förstärkt fadezon. Endast de kanoniska `supporter_tifo_*`- och `supporter_conflict_*`-eventen får respektive 132 px `header` i `EventCardInline`; eventen förblir icke-blockerande inline-kort och övriga supporterhändelser får ingen bild. Galan är orörd enligt designbeslutets nej. Tre deterministiska dev-vyer visar de riktiga produktkomponenterna med text från den verkliga supporterproducenten. Faktiska 390 px-vyer granskade: lampan och tröjan, tifots banderoll och grupp samt konfliktens båda grupper och fallna duk är bevarade. Assetgrinden låser WebP/maxbredd och konsumenttesten låser id→bild samt faktisk `<img>`-wiring. 18/18 fokustester och full produktionsbuild med TypeScript plus fem grindar gröna. Aktiv räknare 3→2. |
| grind2-burnout-aterfall-valtext | klar | cf2e1011 | Tidigare faktiskt burnout-val läses ur liggaren, med sann legacy-fallback och bibehållna poolidentiteter/cooldown. 5 137/5 137 tester och full build/fem grindar gröna. Leverans och avgränsning: playtest/GRIND2_ATGARDSPASS_2026-09-11.md. |
| grind2-berattelse-matchfakta | klar | cf2e1011 | Tävlingsutfall, hemma/borta, januarifönster, scorertext och interaktiv hörnklassificering rättade. Landslagstext lovar inte längre frånvaro som modellen saknar; ingen balansändring. 5 137/5 137 tester och full build/fem grindar gröna. Leverans och avgränsning: playtest/GRIND2_ATGARDSPASS_2026-09-11.md. |
| grind2-inkorg-aktualitet | klar | cf2e1011 | Sann svarsklassificering, introgrind för ämne/röst och direktnotiser, informationsbudget samt match-/tidsankare för uppskjutna kort. Gammal Maria-efterklang lämnar portalen men bevaras i historiken. 5 137/5 137 tester och full build/fem grindar gröna. Leverans och avgränsning: playtest/GRIND2_ATGARDSPASS_2026-09-11.md. |
| grind2-mecenat-samtalsvag | klar | cf2e1011 | Portalen öppnar befintlig trefrågedialog i stället för åtta slutkombinationer; felaktig Eklund-rad borttagen. Browserflöde genom alla steg och test av exakt en slutresolution. Galans designfråga kvar som egen rapporterad post. 5 137/5 137 tester och full build/fem grindar gröna. Leverans och avgränsning: playtest/GRIND2_ATGARDSPASS_2026-09-11.md. |
| grind2-ko-sasongsslut-oloslig | klar | 02088679 | Granska behåller lösta kort som synliga kvitton men fogar nu in varje ny promotion från den uppskjutna kön. Både Översikt- och Spelare-val blockerar fortsättningen tills de är lösta; säsongsrollover pensionerar kvarvarande aktiva och uppskjutna val uttryckligt. Hel testsuite och produktionsbuild gröna. |
| grind2-ko-rolloverflood-semantisk-dedupe | klar | 02088679 | Exakt event-id och valfri semantisk identitet följer nu beslut genom pending, deferred, resolved/expired och den samlade rolloverrapporten. Vardagsjobb är stabilt per spelare och säsong, legacy-period-id blockeras, och varje utfall får kanoniskt kvitto utan notisflood. Hel testsuite och produktionsbuild gröna. |
| grind2-portalbudget-slutspel-fyra-aktiva | klar | 02088679 | Samma räknare omfattar event, veckoval och valbar scen; deadlines prioriteras men skapar aldrig en fjärde plats. Kafferummets interna tryckmätning räknar inte scenen rekursivt. Hel testsuite, femsäsongskalibrering och produktionsbuild gröna utan matchbalansändring. |
| sponsor-motbud-kopromotion-saknas | klar | denna commit | Grind 2:s kodhypotespass hittade att sponsorers båda terminala motbudsutfall frigjorde en aktiv plats utan att köra den kanoniska köpromotionen. `accepted` och `walked_away` promoverar nu `deferredDecisions` direkt efter avslutet; `stood_firm` lämnar avsiktligt kön orörd eftersom erbjudandet fortfarande är öppet. Tre nya store-regressioner täcker samtliga utfall, 42/42 riktade kötester och full build med TypeScript/fem grindar gröna. Aktiv räknare 6→5. |
| sluttest-grind3 | klar | denna commit | Opus dom i `DOM_GRIND_2_3_2026-09-10.md`: Grind 3 passerar utan reservation efter den verkliga tioårskörningen. Huvudloopen höll som primär väg, nästa-frågan var konkret och varierad, och tio managersäsonger gav tio skilda säsongslandningar. Den redaktionella belastningen bröt varken loop eller landningar och dess kända fel är åtgärdade. Aktiv räknare 6→5. |
| arsbok-omgangsetikett-blandform | klar | denna commit | Årsbokens tidslinje skiljer nu på två verkliga skalor utan visuell hoppighet: belagd ligaomgång normaliseras till `Omg N`, medan storyline-poster som bara bär global matchdag får den sanna fallbacken `Dag N`. Äldre lagrad `Omgång N` normaliseras vid läsning; cup- och slutspelsnamn bevaras. Numeriska markörer har en gemensam 58×28 px-yta. Riktig årsbok 2036/37 visade sju etiketter med exakt samma bredd. 7/7 fokustester och full build med TypeScript/fem grindar gröna. Aktiv räknare 7→6. |
| sluttest-validering-journal | klar | denna commit | Den riktiga Grindtest 2-karriären fullföljde tio managersäsonger inklusive avsked, mellanår och ny klubb. Journalens tre delar är kompletta och den intakta saven är säkrad som `docs/playtest/karriar_vastanfors_10sasonger.json`; JSON-syntax, manageridentitet, aktuell säsong 2037 och exakt tio `seasonSummaries` verifierades mot webbläsarens levande save. Underlaget är därmed levererat enligt `TESTINSTRUKTION_KARRIARJOURNAL_2026-09-03.md`. Aktiv räknare 7→6. |
| sasongsmal-barare-raknar-inhopp-och-ny-klubb | klar | denna commit | 2026-09-11: `playerCarry` döms nu mot domens verkliga mått, minst 15 starter i den managerstyrda klubbens färdigspelade matcher, härlett från fixture-elvorna. Inhopp och starter för en köpande klubb räknas inte; verkliga egna starter före en senare försäljning bevaras. 50/50 riktade säsongsmålstester och full build med TypeScript/fem grindar gröna. Aktiv räknare 8→7. |
| gameover-saveexport-saknas | klar | denna commit | 2026-09-11: Game Over har nu en egen nåbar `Exportera säkerhetskopia`-kontroll som anropar samma `exportSaveAsJson` som det ordinarie spelhuvudet; ingen ny lagrings- eller exportväg skapades. Komponenttestet verifierar att exakt den avslutade aktuella saven exporteras. 7/7 riktade Game Over-/historikkontraktstester och full build med TypeScript/fem grindar gröna. Aktiv räknare 8→7. |
| cupforlust-pokalen-pa-byran | klar | denna commit | 2026-09-11: `cup_done`-variantens neutrala `Pokalen står på en byrå någonstans` är ersatt med den tillståndssanna `Pokalen står på någon annans byrå`. Förlustspåret är fortsatt separat från `cup_done_winner`, vars uttryckliga egna pokalinnehav är oförändrat. Routing och båda textpoolerna täcks av 12/12 fokustester; full build med TypeScript/fem grindar grön. Aktiv räknare 8→7. |
| smfinal-dubbelt-uppspel | klar | denna commit | 2026-09-11: finaluppspelets ansvar är nu en scen per matchväg. Full- och referatmatch gatar bort Förbered-dubbletten och behåller `FinalIntroScreen` precis före avslag; snabb- och tystsim, som aldrig går genom liveintrot, behåller `MatchLaddningScene`. Övriga laddningsscener påverkas inte. 41/41 riktade grind-/rotation-/ceremonitester och full produktionsbuild med TypeScript/fem grindar gröna. Aktiv räknare 8→7. |
| arsbok-storyline-dubbleras | klar | denna commit | 2026-09-11: årsbokens frysta `keyMoments` bär nu den kanoniska storyline-identiteten in i skärmen. Den liggarstyrda projektionen får förtur och återanvänder den frysta brödtexten, medan samma identitet filtreras bort ur den andra vägen. Två olika verkliga storylines bevaras även om typ och text är lika. Regressionen reproducerar korsningen; 52/52 fokustester, fullsvit 566 filer/5 084 tester och produktionsbuild med TypeScript/fem grindar gröna. Aktiv räknare 8→7. |
...
| bandygala-aterkomst-efter-durable-receipt | klar | denna commit | Den fjärde galan falsifierade köskyddet som full rotorsak. Granska fryser synliga kort vid mount; när ett tidigare kort löses kan köpromotionen flytta nästa frusna kort från `pendingEvents` till `deferredDecisions`. Resolvern sökte bara pending, så klicket blev en tyst no-op utan valkvitto och galan återkom. `resolveEvent` läser nu båda kanoniska beslutsköerna och pensionerar fortsatt id:t ur båda. 39/39 riktade tester och full produktionsbuild med TypeScript/fem grindar gröna. Verifierat i samma save: ett tidigare val flyttade `event_gala_2033` till deferred, galaklicket löstes därifrån, visade kvittot och aktiverade fortsättningsknappen. Aktiv räknare 8→7. |
| pending-screen-kedja-stannar | klar | denna commit | `DashboardOrPortal` bär nu senast omdirigerad `PendingScreen` i stället för en boolesk vakt. Samma skärm loopar inte, men en direkt `qf_summary` → `season_summary`-kedja får en ny omdirigering. Tre kontraktstester, fyra rollovertester och full build med fem grindar gröna. Den naturligt fastnade sparningen provades i browser: dashboarden öppnade årsboken automatiskt. |
| sluttest-grind2 | klar | denna commit | Grind 2 passerar efter det avgränsade återprovet i `docs/playtest/GRIND_2_ATERPROV_2026-09-11.md`. Ekonomival år 8 och färdigt anläggningsträd var redan bevisade; två aktuella säsonger tränade burnout, kafferum, gala och beslutskö. Provet hittade och rotfixade sponsor-promotion samt pending-skärmkedjan; därefter inga upprepade pivotal-scener eller fastnad kö. 66/66 fokustester, ytterligare 7/7 skärm-/rollovertester och full build gröna. Aktiv räknare 6→4. |
| licensavdrag-text-och-rollover | klar | denna commit | Licensbrevets första poängavdragsvariant säger nu uttryckligen “Tre poängs avdrag inför nästa säsong”. `handleSeasonEnd` aktiverar samma-rollover-licensdomen direkt i den kommande säsongens `pointDeductions` och summerar den med redan väntande skandalavdrag; väntrummet konsumeras i stället för att skjuta licensens −3 ytterligare ett år. 33 licens-/tabelltester och full build med fem grindar gröna. Aktiv räknare 11→10. |
| arsbok-akademi-singular | klar | denna commit | Akademins årsboksrad och Klubbminnets motsvarande kanonläsning använder nu samma `formatSwedishCount`: `1 stjärna`, annars `N stjärnor`. Ingen fryst liggartext ändrades; böjningen sker vid presentation. Sex riktade tester och full build med fem grindar gröna. Aktiv räknare 10→9. |
| arsbok-tidslabel-normalisering | klar | denna commit | Årsbokens tidslinje visar nu `Omg N` bara för belagd ligaomgång och bevarar cup-/slutspelsnamn. Narrative storylines som enbart bär global matchday visar den sannare etiketten `Säsongen` i stället för intern pseudoexakthet som `Dag 16`. Sju riktade tester, full build och browserprov i den verkliga 2038/39-årsboken gröna. Aktiv räknare 9→8. |
| portal-kograf-svarlast | klar | denna commit | Portalens befintliga `fatigueHistory`-sparkline har fått självbärande rubrik och riktning: “Beslutsbörda · senaste omgångarna →” samt separat nuläge “Nu: {Lugn/Märkbart/Hög}”. De tidigare lösa orden “Tryck” och “Lugn” kan inte längre misstolkas som grafens två axeländar. Tre riktade tester, full build och visuellt browserprov i `portal-interruption-budget` gröna. Aktiv räknare 8→7. |
| arsbok-ortsstod-kontext | klar | denna commit | `communityStanding` är verifierat som en giltig 0–100-skala som startar på 50 och kan nå sitt klampade golv i en långkarriär. Årsboken visar nu `{värde} av 100` i stället för en naken siffra och progressbaren bär samma min/max/current semantiskt. Inga orsaker fabriceras när frusen skifthistorik saknas. Åtta riktade tester och full build gröna. Aktiv räknare 7→6. |
| arsbok-managersektion-kontext | klar | denna commit | Årsboken behåller varje fryst managerdagboksrad ordagrant men visar nu dess lagrade `ManagerNarrativeEntry.type` som en lågmäld svensk etikett: bland annat Belastning, Lättnad, Ditt val, Återhämtning och Spår. Ingressen förklarar att sektionen gäller beslut, belastning och återhämtning under året, så tre burnoutfaser inte längre läses som lös melankolisk prosa. Nio riktade tester och full build gröna. Aktiv räknare 6→5. |
| arsbok-hero-integrering | klar | denna commit | Den godkända, sanningsstyrda utfallsbilden är nu bakgrund till årsbokens första resultatblock i stället för ett löst 200 px-band. Resultatet ligger på en svag halvtransparent, lätt suddad platta; motivet förblir synligt runt blocket. Vanligt säsongsslut och faktisk nedflyttning browserverifierades separat vid 390 px, och nedflyttningsbilden styrs fortsatt enbart av tabellens bottenzon. 12/12 riktade tester och full build med fem grindar gröna. Aktiv räknare 5→4. |
| kafferum-derbyeko-saknar-cooldown | klar | denna commit | De fasta `derby_win`- och `big_derby_win`-ekona har nu varsin stabil visningsidentitet och samma tvåsäsongers cooldown som övriga fasta kafferumsekon. Händelsen förblir sann och lagrad; bara den nyligen visade repliken vilar. Vanlig/stor derbyseger, samma/följande/tredje säsong samt hemma-/bortaperspektiv är regressionstestade. 17/17 riktade tester och full build med fem grindar gröna; matchmotorn är orörd. Aktiv räknare 5→4. |
| arsbok-hero-platta-for-opak | klar | denna commit | Årsbokens resultatplatta sänktes från 86 till 68 procent ytfärg, kanten från 65 till 42 procent och bakgrundsoskärpan från 3 till 2 px. Illustrationen läses nu tydligt genom glasytan samtidigt som utfallstexten behåller kontrast. Både vanlig säsongsbild och nedflyttningsbild browserverifierades vid 390 px. 12/12 riktade tester och isolerad full produktionsbuild med fem grindar gröna; huvudträdets samtidiga Inbox-refaktor lämnades orörd. Aktiv räknare 9→8. |
| grind2-gala-gestaltning | klar | `6c01e629` | `event_gala_{season}` (bandyGalaService.ts) renderades tidigare som generisk DecisionCard-textklump — fem liga-priser + hasWinner-gren fanns redan men bar ingen vikt. `GalaScene` ersätter det generiska EventOverlay-fallet: hållen scen (ett pris, `pickHeldAward` väljer nykomling/veteran > övriga bland egna vinnare, annars topp-liga-priset i grå ton) → liggarlandning (resten i protokollform, egna vinnare guldprick) → det verkliga valet (verbatim ur event.choices). Medaljen (🏆, inte mockens 🏒) är den enda fyllda guldytan — mockens `.btn-prim` var guld-gradient, vilket bröt gold-disciplinen (samma regel som SM-final-skarven); byggd flat koppar (--accent-dark) i stället. `--gold-soft` tillagd i colors_and_type.css + global.css (--gold/--gold-deep fanns redan). SVENSK TEXT — CODE SKRIVER ALDRIG: narrativa rader (förspel, award-line, beslutsfråga) är Opus-pending via en tom `galaSceneCopy.ts`-pool; "bittersöt"-tonen kräver ett signal-fält (spelare som säljs/pensioneras) som inte finns vid galans genereringstillfälle och väljs därför aldrig än, dokumenterat i koden. Ingen ny mekanik, ingen balansändring, inget nytt gala-pris. 10 nya tester (pickHeldAward-prioritet/FIFO, GalaScene triumf/grå-gren, gold-disciplin på CTA:n, verbatim choices), browserverifierat (renderToStaticMarkup, båda tonerna). tsc, full build och full testsvit (574 filer/5155 tester) gröna. Ingen baseline ändrad. Beslutskorts-domen (samma körorder) var redan byggd sedan tidigare (`f98456ab`), verifierad intakt, ingen ändring behövdes. |
| grind2-kafferum-fatiguepool-cooldown | klar | `42029869` | Fatigue-selektorn läser nu `narrativeBeatLog` med stabil identitet per faktisk rad och två säsongers cooldown; scenavslutet kvitterar samma identitet i stället för ett kolliderande generiskt poolindex. Hot-läget använder alla 16 redan Opus-godkända fatigue-rader och faller tillbaka till övrigt kafferumsinnehåll när brunnen är på cooldown, utan ändrad frekvensgate. Victory-ekot måste passera sin befintliga cooldown även vid render och pensioneras utan falskt nytt kvitto om det redan ligger på vila. 25 riktade tester, helsvit 5 156/5 156 och full build gröna; femsäsongskalibreringen oförändrad. |
| grind2-langtidsidentitet-tre-konsumenter | klar | denna commit | Kod- och domläsningen delade den felaktigt samlade hypotesen. De två verkliga kafferumsrepriserna är stängda: storsegerpoolens valda rad och den generiska resultatpoolens faktiska rad får stabil visningsidentitet och två säsongers cooldown i `narrativeBeatLog`; när alla resultatrader vilar lämnas plats åt annat material. Galan förblir avsiktligt årlig (`event_gala_{season}`), vilket det senare godkända Grind 2-provet uttryckligen räknar som PASS vid exakt en gala per säsong. `WeeklyDecision.repeatPolicy` dokumenterar på samma sätt att standardbeslut får återkomma en senare säsong; bara `untilAccepted` är permanent. Ingen kanonisk händelse flyttades till cooldown-lagret och ingen match-/balansparameter ändrades. 27/27 riktade tester och full produktionsbuild med TypeScript/fem grindar gröna. Aktiv räknare 8→7. |
| grind2-nykarriar-kompletterande-aterprov | stale | `d863800a` | Det efterfrågade nya tvåsäsongsprovet var redan genomfört efter blockerfixarna och ratificerat i `docs/DOM_GRIND2_ATERPROV_3_2026-09-11.md`: naturlig Heros-karriär över två säsonger, burnout pressat till terminalval, högst två aktiva och två uppskjutna beslut, inga fastnade köer eller reprisbrott samt riktig 390 px-verifiering av säsongsslut utan slutspel. Domen är `PASS, noll invariantbrott`. Djupt-slutspel-ekot nåddes inte och står i domen som separat icke-blockerande uppföljning, inte som skäl att upprepa hela provet. Aktiv räknare 7→6. |
| arrival-topnav-kontrast-mobil | klar | `42029869` | Ankomstens genreetikett, progress-streck och skip-länk har fått en mörk varm hierarki, lätt halo och en diskret 118 px backing som endast ligger över bildens topp. Verifierad på Rögle-introt i faktisk 390×844-render: samtliga kontroller läsbara mot himlen utan att konkurrera med huvudkortet. Helsvit 5 156/5 156 och full build/designgrindar gröna. |
| grind2-arsbok-styrelsekrav-motsags | klar | `1b213092` | Årsboken fryser nu samma tabellbundna BoardObjective-resolution som portal och säsongsslutets inkorg. Ett konkret ”Sluta topp 6 — misslyckat” prioriteras i hero, färg, ikon, placeringsdom och narrativ framför den grövre MidTable-skalan; ägarreaktionen och rubriken återanvänds från samma objektiv, utan ny tröskel eller balansändring. Regressionen reproducerar exakt plats 8 där den gamla skalan fortfarande är `exceeded` men årsboken korrekt dömer topp-6-målet som `failed`. Befintlig storyline-identitetsdedupe återprovad för dubblad derbytriumf. 32/32 fokustester, helsvit 575 filer/5 159 tester och full build med samtliga design-/innehållsgrindar gröna; femsäsongskalibreringen oförändrad på 9,39 mål/match och 21,9 % hörnmål. |
| grind2-anslag-dismiss-laser-portalen | klar | `593fa055` | Overlayns fokus- och inertlivscykel binds inte längre om vid varje portalrender; dismiss anropar den senaste callbacken. Regression täcker callbackbyte och Escape, befintligt tillgänglighetstest passerar, och Annandagsanslaget klickades utan konsolfel i riktig mobil dev-scen. Aktiv räknare 10→9. |
| grind2-motstandarspelare-eget-klubbval | klar | `593fa055` | Karaktärsspelarproducenten kräver nu managedClubId och filtrerar kandidater före generering. Regression med samtidiga hungriga egna och motståndarspelare visar att bara den egna får ett val. Aktiv räknare 9→8. |
| grind2-arsbok-objektiv-sanningssplit | klar | `593fa055` | Nästa säsongs mål skapas och utvärderas mot den nya säsongens fixtures, tabell, cupstate, rivalitetshistorik och nollställda räknare. Årsbokens hero använder den frusna objektivdomen och kan inte vara grön när ett uppdrag är missat. Regressioner täcker cup-/derbyprogress och missat uppdrag; 25/25 riktade årsbokstester och full build/grindar passerar. Aktiv räknare 8→7. |
| grind2-transferbud-avvisat-och-pending | klar | `593fa055` | Transferprocessorn kollapsar samma deterministiska bud-id före behandling, låter terminal status vinna över pending och blockerar nygenerering mot alla kända id:n. Exakt rejected+pending-checkpoint ger nu bara rejected och inget nytt bud. Aktiv räknare 7→6. |
| inv-5-fas4-klubbmarken | klar | denna commit | Samtliga tolv spelbara klubbar har fått frilagda RGBA-mastrar med äkta alfa samt självständiga inbäddade 16/32/64-SVG:er. `ClubBadge` väljer mikrovariant vid ≤24 px, kompakt variant vid 25–40 px och full variant på större ytor; deterministisk legacy-fallback finns endast för okända framtida klubb-id:n. Källoriginal, transparenta mastrar, fryst kontaktkarta och storlekskontrakt är bevarade i designsystemet. Gagnef, Heros och Söderfors underkändes först som RGB med bakgrund och gjordes om innan wiring. Äldre bokstavssköldar i Tillträdet och båda finalpresentationerna ersattes. Bildytorna använder en gemensam kontrastplatta, browsergranskad mot samtliga tolv klubbillustrationer. Alla 36 produkt-SVG:er samt bägge finalflödena täcks av den permanenta imageWiring-grinden; 19/19 riktade komponent-/assettester, full TypeScript-/produktionsbuild och samtliga fem buildgrindar gröna. Aktiv räknare 10→9. |
| rc-sasongsrollover-akademi-skada | klar | denna commit | Den hanterade klubbens sommarintag kommer nu endast ur den verkliga P19-truppen; samma spelare och id:n matar inbox, den dual-writade intagshistoriken, eventLedger och årsboken, men når seniortruppen först genom den befintliga academy-promotion-pipelinen. AI-klubbarna behåller sin abstrakta föryngring eftersom de saknar simulerad P19-trupp. Säsongsrollovern minskar samtidigt `injuryDaysRemaining` med verkliga kalenderdagar fram till 1 oktober, synkar `isInjured` och rensar tillfälligt spela-på-läge. 95/95 breda säsongsslutstester, TypeScript och full build med fem grindar gröna. Aktiv räknare 9→8. |
| rc-arsbok-komplett-sanningsunderlag | klar | denna commit | Årsbokens spelarpris läser nu betyg från samtliga färdigspelade matcher för den hanterade klubben, inte bara derby-, storseger- och senmatcher som råkade överleva komprimeringen. Den hanterade sidans fulla matchtrupp bevaras också under den pågående säsongen, så inhoppare med betyg kan räknas; motståndarnas bänkar och AI-matchernas betyg komprimeras fortsatt och hela fixturemängden ersätts vid rollover. Styrelse- och licensmotsägelserna var redan separat rotfixade och arkiverade. 56/56 riktade tester, full produktionsbuild med fem grindar och hela sviten 578 filer/5 195 tester gröna. Aktiv räknare 6→5. |
| rc-kontext-sprak-grafik-brett-svep | klar | denna commit | Rapportens nådda fel reconcilierades mot aktuell kod och tidigare fixar. Kvarvarande glapp rättades: slutspelsläget säger “En förlust till”, finalen böjer “ett derby”, kall klaris väljer inte den motsägande trög-boll-poolen, hemmaevent kan inte välja bortabussen och den motsägande juniorrepliken pensionerades, akademins investering/drift namnges, kafferummets CTA visar en pil och januarianslaget använder den generiska bruksortsbilden i stället för annandagsfesten. Straff-/tävlingsaxel, matchankring för fördröjda kort, motståndarcitatets klubb/roll, domarerfarenhet, singular, arenaformat och övriga listade språkfel var redan kod- och testverifierat lösta. 107/107 riktade tester, full produktionsbuild med fem grindar samt mobil browserkontroll av januari och kafferum gröna. Aktiv räknare 5→4. |
| lokalhjalte-mal-hemmaplanscopy-borta | klar | denna commit | Lokaltraitets målpool saknar venue som indata och används därför både hemma och borta. Den enda felaktigt hemmaplansbundna varianten är ersatt med Opus levererade “Sånt bär han med sig vart laget än spelar”; övriga två var redan venue-neutrala. Ett test väljer samtliga tre poolpositioner och förbjuder hemmaplanspåståendet. 27/27 riktade motor-/kommentarstester och full produktionsbuild med fem grindar gröna. Aktiv räknare 4→3. |
| inv-4-o12-forhandsdelta | klar | denna commit | O12-domarmötet är nu ett verkligt tvåaxligt val enligt `DOM_O12_DOMARMOTE_AVVAGNING_2026-09-11`: Respektera ger domarrelation +1 och klackstämning −2, Neutral är no-op, Protestera ger domarrelation −1 och klackstämning +2. De tre Opus-raderna visas ordagrant före valet. Båda faktiska, clampade deltan sparas i det strukturerade O12-kvittot; den gemensamma orsak/verkan-liggaren bär också båda axlarna till den synliga “DET DU VALDE”-raden. Befintliga domartrösklar, fejd/förtroende-poster och utgångspolicy bevaras genom en delad relationsskrivväg. 40/40 riktade tester, TypeScript och full produktionsbuild med fem grindar gröna. Browserprovat vid 390 px före och efter Respektera: “Klacken knappt ner, Domaren knappt upp.” Aktiv räknare 3→2. |
| o12-mecenatintro-dominant | klar | denna commit | O12-återprovet skilde den verkliga mecenatproducenten från den redan fixade patronproducenten. Det strikt sämre försiktiga ja-svaret (+5 utan motkostnad) är borttaget; det verkliga acceptera/avböj-valet står kvar. Save-migreringen rensar samma svar ur både `pendingEvents` och `deferredDecisions`, och schema 0.3.13 aktiverar migreringen för befintliga saves. En riktig legacy-save gick i browser från tre till två val och löstes vidare till nästa kökort. 5 207/5 207 tester och full produktionsbuild gröna. O12:s populationsgrind är separat och kräver fortfarande oberoende naturliga val. Aktiv räknare 18→17. |
| genomgang-opus-edits-commit | klar | 508f47fc | Pass 0 (CODE_KORORDER_GENOMGANG_2026-09-12, §0). Femton Opus-edits i arbetsträdet verifierade och committade i sju rotorsaks-grupperade commits: server (`088ca5cf`, trust proxy + timingSafeEqual), persistens (`e9d971d8`, snapshot-rotation per orsak), PWA (`2684e52b`, precache exkl. porträtt/illustrationer + WCAG-zoom, verifierat i dist/sw.js), a11y (`dd294c38`, SectionLabel aria-hidden), motor/store (`9d3ac12c`, MatchLiveScreen-kontext, pickBestEleven, signFreeAgent-dubbelpost + uniquePlayerIds-invariant, wentToOvertime, kaptensbindel), CI (`7f320a4a`, Node 22), docs (`508f47fc`, CLAUDE.md klubbnamnsrättelse + ny README). Fulla sviten 580/580 filer, 5207/5207 tester, tsc och build gröna innan och efter. Aktiv räknare 17→16. |
| genomgang-illustrationer-webp | klar | 92abe779 | Pass 1 (CODE_KORORDER_GENOMGANG_2026-09-12, §2). scripts/convert-illustrations.mjs (sharp, q80/1170px) konverterade 23 JPG → webp, original raderade. 10,90 MB → 2,64 MB för de konverterade filerna (mappen totalt 12 MB → 3,7 MB inkl. tolv redan-webp klubbintron, orörda). Referenser bytta i IllustrationScene.tsx, MiljoHeader.tsx, MatchLaddningScene.tsx, FinalIntroScreen.tsx, CeremonyCupFinal.tsx, CupFinalVictoryScene.tsx, imageAssetIntegrity.test.ts + matchande testförväntningar. Browserprov (dev/scenes, Playwright, 390px): finalhelg/season-header/game-over/coffee-room laddade sina webp korrekt, complete:true, korrekta naturalWidth/Height. 580/580 filer, 5207/5207 tester gröna. |
| genomgang-code-splitting | klar | 934af98b + b949884d + e3a4eaef | Pass 2 (§1) med korrigerande rotpass. Första två committerna gjorde åtta tunga skärmar och MatchLive lazy samt flyttade den vanliga advance-/abandoned-match-vägen bakom dynamisk import, men den arkiverade mätningen 1,93 MB beskrevs felaktigt som under 1,5 MB. Roten var att globala `careerBreakActions` fortfarande toppimporterade `simulateCareerBreak`, som statiskt drog in roundProcessor → matchSimulator → matchCore; därför upphävdes den andra lazy-gränsen. `e3a4eaef` laddar i stället hela career-break-use-caset med EN dynamisk import vid det faktiska valet (inte den gamla deadlockande `Promise.all`-varianten inne i use-caset), väntar in asynkrona advance-anrop, gör Match/Club/Granska route-lazy och skiljer stabil vendor-kod från produktens startup-chunk. Aktuell Vite 6-build på main: 2 294,77 → 1 395,50 kB (gzip 437,23 kB), alltså faktiskt under det beställda 1,5 MB-taket; matchCore ligger separat på 40,68 kB. 25/25 riktade karriäruppehålls-/omgångstester och full build med fem grindar gröna. Ingen match-, balans- eller save-data ändrad. |
| genomgang-matchprofile-halvlek | klar | denna commit | Matchens grundprofil rullas nu exakt en gång och bärs som uttrycklig state över fast simulering, halvtid och samtliga interaktiva regenereringsvägar. Ett defensivt matchförlopp kan därför inte bli kaotiskt bara för att andra halvlek får en ny RNG-seed. Regressionen tvingar olika seedprofil men kräver oförändrad `defensive_battle`; 31/31 fokustester, TypeScript och full produktionsbuild med fem grindar gröna. Separat stressprov 6 843 matcher låg kvar på utvisningsfixens 8,98 mål/match, alltså ingen ny kalibreringsdrift. Aktiv räknare 15→14. |
| genomgang-contentcontract-bundle | klar | 934af98b + denna commit | Den öppna raden var en kvarlämnad dubblett till det redan arkiverade code-splitting-passet. `contentContractRuntime.ts` innehåller den lilla id/source/whyNow-projektion produkten behöver; EventOverlay och eventQueueService importerar endast runtime-filen. Det fulla 266 kB-registret står kvar som granskningskälla utanför produktionsgrafen, och kontraktstestet låser att projektionerna inte glider isär. 26/26 kontraktstester och full produktionsbuild med fem grindar gröna. Ingen ny produktkod behövdes. Aktiv räknare 14→13. |
| genomgang-match-dod-kod | klar | f6069d65 + denna commit | Importinventeringen bekräftade att `useMatchTimer` och `useMatchGenerator` var 348 rader oimporterade kopior av MatchLiveScreen och de är borttagna. `INTERACTIVE_CORNER` hade noll avsändare och var en dokumenterad no-op, så action och reducergren är borta. Utvisningsstate över halvtid var redan rotfixad i `1b0528e3`; reducerprovet låser nu att en aktiv utvisning står kvar. Half och Midfielder är avsiktligt skilda roller i Formation, motor och UI; den verkliga buggen var en överspelad `loadGame`-migrering som kollapsade varje mittfältare till halv även i nya saves. Den är borttagen och en tvåslotts-save-rundtur låser identiteten. 29/29 riktade tester och full Vite 6-produktionsbuild med fem grindar gröna. Helsviten 5 221/5 222; enda röda är den redan kända, isolerat reproducerade `seasonRolloverStaleEvents`-mecenatposten utanför passets filer. Aktiv räknare 13→12. |
| arsbok-toppbetyg-matchgolv | klar | denna commit | Årsbokens toppbetyg kräver nu betyg i minst halva den faktiskt spelade ligasäsongen, med fem matcher som golv för korta eller avbrutna säsonger. Snittet förblir oviktat och läsbart. Regressionen bygger exakt 22 ligamatcher och bevisar att en spelare med nio perfekta betyg inte slår en spelare med 22 stabila betyg. 51/51 riktade årsbokstester, separat TypeScript-kontroll och full produktionsbuild med fem grindar gröna. Aktiv räknare 12→11. |
| genomgang-jacob-beslut | klar | denna commit | Jacobs två öppna vägval är nu kanoniskt bokförda. Bandygrytans källdokument och fyra JSON-underlag stannar spårade i repot. Release-definitionen anger två mätbara kriterier inom två veckor: båda testarna ska nå minst en `season_completed`, och medianen för `session_end.durationSeconds` ska överstiga 15 minuter. Kriterierna dömer soft-launchen efteråt och blockerar inte publicering. Ingen produktkod eller teständring krävdes. Aktiv räknare 11→10. |
| genomgang-portal-effekter | klar | denna commit | Portalens fem verifierade hygienfynd är stängda utan ändrad spelmekanik. Automatisk hoppning över främmande matchdagar har en StrictMode-säker engångsvakt; CTA-höjdens ResizeObserver binds bara om när den villkorliga CTA-ytan byter läge; PortalScreen muterar inte längre `body`, utan GameShell äger portalbakgrunden; hela cup-/slutspels-/åskådarbeslutet för CTA-etiketten ligger som ren funktion bredvid `getNextActionCue`; BottomNav återbrukar designsystemets `texture-wood` i stället för en rå rgba-kopia. 35/35 fokustester, TypeScript och full produktionsbuild med fem grindar gröna. Browserprov vid 390 px: rätt omgångs-CTA, ingen horisontell overflow och tom `body.style.background`. Aktiv räknare 10→9. |
| genomgang-toolchain-bump | klar | `8cf4ead9` + `b19c655a` + `ccbd615c` + `fd3f44c5` | Uppgraderingen landade stegvis och separat: `@types/node` 22.20.2, Vitest 3.2.7 och Vite 6.4.3. Samma tio seedar över fem säsonger före/efter gav 50/50 genomförda säsonger, 6 348 matcher, noll krascher och byte-identiskt normaliserat utfall; de åtta ekonomi-varningarna var samma på båda sidor. Produktionsbygget och fem grindar är gröna; startup-chunken är 1 395,46 kB (gzip 437,21) och `matchCore` separat 40,68 kB. Den kompletta visuella körningen gav 777/778 före ett rent 30-sekunders timeout i 24-sidors klubbsvepet; samma test passerade 1/1 på 22,2 sekunder med lokal 90-sekundersbudget. Linux behåller pixeljämförelser medan macOS kör samtliga strukturella grindar utan falska Darwin-baselines. Auditens kvarvarande fyra fynd är moderata. Aktiv räknare 9→8. |
| genomgang-motor-attribut-aldras-ej | klar | denna commit | CODE_KORORDER_2026-09-12_EM rad 1. `applyVeteranAttributeDecline` (playerDevelopmentService.ts) applicerar developPlayers egen nedgångsgren (samma baseChange-formel, samma slumpmässiga extra-nedgångsattribut) på managed-klubbens 31+-åringar vid säsongsslut — rör bara `attributes`, aldrig `currentAbility` (omgångssystemet äger den). En enda applicering per säsongsslut gav bara 1/11 av AI:s nedgång på identiska 34-åringar (uppmätt: 0,09 mot 1,01 attributpoäng-snitt över tre säsonger) — löst genom att köra samma formel `Math.floor(spelade_omgångar/2)` gånger per säsongsslut, exakt AI:s egen `nextRound % 2 === 0`-takt (seasonEndProcessor.ts räknar detta ur faktiskt spelade omgångar, ingen gissad konstant). Placerad efter årets sista applyRoundDevelopment-körning, före updateActiveLegendFlags. Riktat enhetstest bekräftar jämförbar nedgång (0,95 mot 1,01) mellan identiska managed/AI-veteraner. Stress 10 seedar × 6 säsonger: 0 krascher, goalsPerMatch 8,97 (ingen ny drift utöver den redan Jacob-godkända suspensionstimer-glidningen). 12/12 riktade tester, TypeScript och full build gröna. Aktiv räknare 8→7. |
| genomgang-portal-cue-typografi | klar | denna commit | Portalens “Vad nu?”-cue är handlingshjälp, inte citat eller versal label, och använder nu den etablerade `.h-body-sm`-rollen i stället för en lokal 10,5 px-avvikelse. Den dynamiska varnings-/sekundärfärgen bevaras som lokal modifierare. 25/25 fokustester, separat TypeScript-kontroll och full Vite 6-produktionsbuild med fem grindar gröna. Browserprov i 390 px-scenen bekräftade `.h-body-sm`, beräknad 11 px text och ingen horisontell overflow. Aktiv räknare 7→6. |
| genomgang-motor-utvisning-halvlek | klar | `1b0528e3` + `88fde9f7` | Utvisningens exakta återstående tid bärs över halvtid, regenerering och förlängning; pausen förbrukar ingen tid och legacy-state får en ändlig timer. Den föreskrivna `calibrate:firing:formal` visade sig mäta avskeds-/sparkfrekvens och saknade helt utvisningsdata; fel verktyg stoppades efter sex shards. En reproducerbar `calibrate:suspensions:formal` mäter i stället båda lagens utvisningar över 10 000 parade seedar, fyra taktiska/positionsprofiler, fyra motståndsnivåer och två väderlägen. Utfallet blev 139 729 utvisningar på 40 000 matcher, **3,493/match**, under stoppgränsen 3,85. Varken `SUSPENSION_FREQUENCY_MOD` eller `GOAL_RATE_MOD` ändrades. Den oanvända parallella `MATCH_ENGINE_VERSION` togs bort; `ENGINE_VERSION` är ensam kanonisk version. Aktiv räknare 6→5. |
| genomgang-motor-smafynd | klar | denna commit | CODE_KORORDER_2026-09-12_EM §13, sex punkter. (1) `matchEngine.ts`: penaltiesHome/Away var hårdkodade 0 — räknas nu från `MatchEventType.Penalty`-eventen per klubb, samma mönster som savesHome/Away. (2) `matchCore.ts`: `lastMinutePressData.fatigueLevel` läste `100−morale` — bytt till `100−fitness` (moral och kondition är skilda fält, fatigue ska vara kondition). (3) Två `ENGINE_VERSION`-konstanter var redan dedupade av `genomgang-motor-utvisning-halvlek` (raden ovan) innan detta pass nådde filen — verifierat att `ENGINE_VERSION` (inte `MATCH_ENGINE_VERSION`) är den som faktiskt importeras av `scripts/data-warehouse/{generate,validate}.ts`, ingen ytterligare ändring behövdes eller gjordes. (4) `MatchLiveScreen.tsx`: `resolveAIPenaltyKeeperDive` fick hårdkodad `'offensive'` — läser nu motståndarlagets faktiska `tactic.mentality`. (5) `seasonEndProcessor.ts`: AI-klubbarnas nya ungdomsintag (genererat för NÄSTA säsong) gick tidigare igenom samma reset-mapp som existerande spelare och fick +1 ålder och en säsongshistorik-post för en säsong de aldrig spelade — läggs nu till EFTER reset-mappen. (6) Störst: `transferState.freeAgents: Player[]` var en FULLSTÄNDIG kopia av spelare som redan låg i `game.players` (`clubId==='free_agent'`) — kopiorna åldrades aldrig medan originalen gjorde det, roten till dubbelposten `signFreeAgent` tidigare täppte med en `alreadyInPlayers`-koll. Mätt på en riktig 10-säsongerskarriär (`karriar_vastanfors_10sasonger.json`): 7,48 MB totalt, `freeAgents` ensamt 3,97 MB (1359 poster, 53 %). Ersatt med `transferState.freeAgentIds: string[]` + `resolveFreeAgents(game)`-projektionen i `transferService.ts`; `freeAgentSince` stämplas nu på spelarposten direkt (fanns redan på `Player`) i stället för en separat kopia, och en fri agent som är 37+ eller stått olottad två säsonger gallras nu bort helt ur `game.players` i stället för att bara döljas i en display-lista (den gamla gallringen filtrerade aldrig bort originalet). Migrering tillagd i `saveGameMigration.ts` för gamla saves (flyttar orphanade spelare in i `game.players`, konverterar till id-lista). Sex riktade testfiler (transferActions, transferService, playerDevelopmentService) 65/66 gröna — enda röda är den redan kända, isolerat reproducerade `seasonRolloverStaleEvents`-mecenatposten (bekräftat oberoende av detta pass: identiskt fel även med `seasonEndProcessor.ts` återställd till `HEAD`). Separat TypeScript-kontroll ren. Kopplat: `genomgang-save-storlek-matrad` uppdaterad med mätningen och hålls öppen (freeAgents-borttaget ger ~3,5 MB kvar, fortfarande över 2 MB-tröskeln). Aktiv räknare 5→4. |
| genomgang-save-storlek-matrad | klar | denna commit | Codes färska tiosäsongsmätning efter free-agent-fixen gav **1,45 MB** totalt: `transferState` minskade från **3,97 MB till 5,1 kB**, medan `players` stod för **760 kB utan dubbletter**. En oberoende kontrollkörning med en helt ny Västanfors-save genom tio riktiga säsongsrollovers gav 1 602 986 byte (1,603 MB); saven innehöll 10 `seasonSummaries`, 375 spelare, 785 liggarposter och 137 `freeAgentIds`, och legacy-`freeAgents` saknades. Kontrollkörningens största fält var `players` 776 346 byte, `eventLedger` 240 132 och `narrativeBeatLog` 162 852; `freeAgentIds` tog 4 863 byte. Båda mätningarna ligger under 2 MB-gränsen och den tidigare dominerande bidragsgivaren är borta. Ingen players-spec, följdspec eller ytterligare produktändring behövs. Aktiv räknare 5→4. |
| inv-5-fas5-portrattgenerator | klar | denna commit | Young 7–30 har verifierats mot det låsta porträttregistret och produktbehandlats genom den etablerade beskärnings-, friläggnings- och komprimeringspipelinen till 24 transparenta 400×400 RGBA-PNG:er (2,55 MiB). `CURATED_PORTRAIT_INDICES.young` väljer nu deterministiskt ur hela 1–30-poolen och asset-URL:ens cacheversion är höjd så de ersatta 7/8-filerna inte fastnar hos spelare. Den svaga 31-polynomialhashen gav samma young_11 åt två olika spelare vars strukturerade id:n kolliderade modulo 30; porträttvalet använder nu det generiska FNV-1a-fröet och samma UI-prov ger young_6 respektive young_18. Samtliga 106 högupplösta original för ung/mid/erfaren/veteran är bevarade kanoniskt i originalarkivet. Kontaktkarta på både ljus och mörk yta samt faktisk 390 px-truppvy granskades utan bruten länk, exportkant eller konsolfel. Assetgrinden låser nu även PNG-format, 400×400 och alfa för varje valbart porträtt. 8/8 fokustester, TypeScript och full produktionsbuild med fem grindar gröna. Fullsviten nådde 5229/5230; enda röda testet är den sedan tidigare kända, isolerade mecenatposten i `seasonRolloverStaleEvents.test.ts`, utanför porträttpassets filer. Aktiv räknare 4→3. |
