...
| genomgang-code-splitting | klar | 934af98b + b949884d | Pass 2 (§1) + uppföljning samma dag. React.lazy + Suspense på åtta skärmar (SquadScreen, TransfersScreen, TabellScreen, SeasonSummaryScreen, InboxScreen, HistoryScreen, GameOverScreen, TilltradeScreen) + MatchLiveScreen som egen chunk. contentContract.ts splittad i contentContractRuntime.ts (bara whyNow-lookupen produktionskoden läser). Uppföljning (genomgang-store-lazy-matchcore, Jacobs order efter granskning av 2,27 MB-resultatet): gameFlowActions.ts:s advance()/simulateRemainingStep() och matchActions.ts:s simulateAbandonedMatch dynamiskt importerar matchmotorkedjan i stället för toppimport — en tredje väg (simulateCareerBreak.ts, careerBreakActions.ts) reverterades medvetet efter ett verkligt deadlock (Promise.all mot två överlappande dynamiska importer hängde >120s i vitest), dokumenterat i careerBreakActions.ts, kvar statisk. Huvudchunk 2,79 MB → 1,93 MB (gzip 606 kB) totalt över båda passen. **Under det ursprungliga 1,5 MB-målet** — resterande bulk är eventResolver.ts (149 kB, all eventupplösning oavsett skärm) och liknande korsskärande Portal-domänservice, uttryckligen utanför denna radens scope (Jacobs egen avgränsning). 581/581 filer, 5217/5218 tester gröna (1 pre-existing röd, seasonRolloverStaleEvents.test.ts, spårad till en samtidig agents motorarbete i matchCore.ts/matchEngine.ts, ej i denna radens ändrade filer). |
