...
| pt6-nedslackning-timing | stale | denna commit | PT-6 skrevs 2026-07-13 som en lågprioriterad timingnot utan identifierad komponent. Dagens Stålvallen-vy släcker inte matchhändelser: `CommentaryFeedStalvallen` bevarar dem i en scrollbar matchlogg och har ingen borttagnings-/opacity-timer. De kvarvarande 2,5/4-sekunders tiderna gäller interaktionsutfall; 4 sekunder vid mål beslutades senare uttryckligen i `7d76090a` för att panelen och resultattavlans målblinkning ska sluta samtidigt. Ingen levande motsvarighet till den gamla noten återstår och ingen timing ändrades. |
...
| c-sp5-smfinal-skarv | stale | denna commit | Den aktiva raden hade bara jämförts mot den tidiga crossfade-fixen `0e39f249`. Senare `c965becf` gjorde finaluppspelet fixed/inset med egen heltäckande scenbakgrund och synkad fade, och `837480e6` adresserade uttryckligen den kvarvarande svart/grå roten: båda SM-finalstegens kalla CTA-fotband gjordes transparenta mot `--bg-scene`. Dagens `FinalIntroScreen`, `MatchLaddningScene` och `MatchLiveScreen` behåller skydden; ingen ytterligare CSS-fix är motiverad. |
