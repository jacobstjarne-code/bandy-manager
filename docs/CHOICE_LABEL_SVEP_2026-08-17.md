...
**Ingen ytterligare no-op hittad.** Andra körningen (6 seeds × 10 säsonger, ~4m21s, EFTER mecenat-fixen): **10 899 events sedda (33 distinkta typer), 12 183 choices probade, 0 kast.** Detta är ett positivt resultat i sig — de fyra manuellt hittade no-opsen från runda 1 plus mecenat-fyndet ovan verkar vara HELA no-op-klassen som fanns, inte toppen av ett isberg. Se `_meta.distinctEventTypes` i JSON-filen (skrivs över vid varje körning, denna sessions sista körning är den som ligger kvar) för exakt täckning.
