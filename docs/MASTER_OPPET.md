...
**14 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `genomgang-matchprofile-halvlek` stängdes efter fryst profil genom halvtid och regenerering, 15→14.
...
| genomgang-contentcontract-bundle | `contentContract.ts` 267 kB dokumentationsprosa i produktionsbunten; bara whyNow läses i runtime | in_progress | Codex | §8 | CLAIM 2026-09-12T10:08:32+0200 — Codex. Kod och git verifierar att `934af98b` redan delade ut `contentContractRuntime.ts`; produktionsimporterna läser den lilla runtime-filen medan registerfilen och dess TypeScript-assertioner står kvar. Reconcile som redan löst dubblettrad. |
