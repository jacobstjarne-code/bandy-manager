...
| id | beskrivning | status | ägare | källa | nästa-åtgärd |
|---|---|---|---|---|---|
| o10-queryparam-clubselection | Query-param-läsning på club-selection (seed i länk) inte byggd | in_progress | Code | BACKLOG.md:55 | CLAIM 2026-09-09T09:14:46+02:00 — Code. ClubSelectionScreen.tsx:18-20 — Date.now()-seed, ingen useSearchParams. **→ GO 2026-09-08 (Jacob): bygg seed-i-länk-skivan (useSearchParams på ClubSelectionScreen, läs seed ur query-param). Låser upp grind 4 + delbarheten. Texten låst (`o10-delningskort-text`). Ägare Code. Byggbar.** |
