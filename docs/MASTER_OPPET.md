...
| taktik-autofyll-knappar-trafyta | Taktiktavlans tre autofyll-knappar ("Starkast"/"Mest utvilad"/"Bäst för dagens match") är 35px höga — under 44px-tumträffgolvet | in_progress | Codex | tests/visual/mobileDecisionHierarchy.visual.ts (`findControlSizeViolations`, taktik-testet) | **→ CLAIM 2026-09-10T13:10:00+02:00 — Codex. Jacob har valt Code-fixen: höj till 44px, verifiera hela treknappsraden vid 390×844 och dokumentera inte ett undantag.** [Verifierat 2026-09-10 (Code, m5-sub-pixel-passet): `git log` visar `minHeight: 35` i `FormationView.tsx` introducerad i `0fb32422` (tidigare session), oförändrad av m5-passet. Testet failar konsekvent: "Starkast" 109×35px, "Mest utvilad" 109×35px, "Bäst för dagens match" 107×35px — alla under 44×44 min. Orelaterat till m5 (textstorlek); egen kontrollstorleks-regression. Ingen fix försökt — hittad som sidoeffekt, inte adresserad.] |
