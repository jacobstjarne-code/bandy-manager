...
**11 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `arsbok-toppbetyg-matchgolv` stängdes med ett dynamiskt halvsäsongsgolv, 12→11.
...
| genomgang-jacob-beslut | (a) bandygrytan-datan i repot eller gitignorad — **AVGJORD 2026-09-12: STANNAR I REPOT.** Jacobs dom: icke-problem i praktiken, ingen persondata; databasskydds-risken vägs inte tyngre än friktionen att flytta. Ingen åtgärd. (b) mätbart soft-launch-mål — **AVGJORD 2026-09-12, se release-definitionen** | in_progress | Code | §Jacob-beslut | CLAIM 2026-09-12T10:34:06+02:00 — Code. Kod- och diskkontroll: Bandygrytans fem data-/källfiler är spårade i repot enligt Jacobs beslut. Release-definitionen innehåller de två beslutade tvåveckorsmålen: båda testarna når `season_completed ≥ 1` och median `session_end.durationSeconds > 15 min`. Ingen produktkod återstår; arkivera beslutsposten tillsammans med release-definitionen. |
