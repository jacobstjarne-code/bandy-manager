...
**3 aktiva poster:** `stickiness-drift-backend`, `begriplighet-e-flikintroduktioner`, `begriplighet-f-granska-eko-yta`. `begriplighet-klass-h-kartlaggning` stängdes 2026-09-15 efter att Opus levererade den spelarvända termen "din karriärhistorik" och Code wirade den med regressionstest; se ARKIV. Räkna endast faktiska datarader med status `rapporterad`, `verifierad` eller `in_progress`; historiska räknarändringar och stängda poster finns i `docs/MASTER_ARKIV.md`.
