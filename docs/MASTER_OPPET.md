...
**4 aktiva poster:** `stickiness-drift-backend`, `begriplighet-klass-h-kartlaggning`, `begriplighet-e-flikintroduktioner`, `begriplighet-f-granska-eko-yta`. `begriplighet-b-flerommgangshopp-financelog` stängdes 2026-09-15 — visade sig vara en falsk positiv i `checkFinanceLogGap` själv (single-round-check missade multi-omgångshopp), inte ett spelfel; invarianten omskriven till en range-check, se ARKIV. Räkna endast faktiska datarader med status `rapporterad`, `verifierad` eller `in_progress`; historiska räknarändringar och stängda poster finns i `docs/MASTER_ARKIV.md`.
