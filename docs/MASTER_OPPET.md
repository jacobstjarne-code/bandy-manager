...
**15 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `arsbok-toppbetyg-matchgolv` registrerades för Jacobs dom, 14→15.
...
| arsbok-toppbetyg-matchgolv | Årsbokens toppbetyg kan räknas ur en komprimerad restmängd matcher när vardagsmatcher tömts men derbyn/storsegrar ligger kvar; då riskerar årsboken att presentera ett missvisande toppbetyg | rapporterad | Jacob | Brett tvåsäsongsprov / Jacobs order 2026-09-12 | Jacob dömer om årsboken ska kräva ett minsta matchunderlag och vilket golv som i så fall gäller. Ingen kodändring före dom. |
...
| genomgang-jacob-beslut | (a) bandygrytan-datan i repot eller gitignorad, (b) mätbart soft-launch-mål ur analytics-röret | rapporterad | Jacob | §Jacob-beslut | Två ja/nej. Inget Code-arbete förrän besluten är tagna. |
| genomgang-motor-attribut-aldras-ej | Hanterade klubbens spelare får ALDRIG attributnedgång med ålder (developPlayers är AI-only; applyRoundDevelopment rör bara CA; motorn läser attribut). AI-veteraner sjunker, dina inte. | rapporterad | Jacob→Code | §7 | Jacob dömer förslaget (nedgångsfas enbart, CA-del av); Code mäter före/efter med stress över 6 säsonger. Ingen konstant gissas. |
| genomgang-contentcontract-bundle | `contentContract.ts` 267 kB dokumentationsprosa i produktionsbunten; bara whyNow läses i runtime | rapporterad | Code | §8 | Dela i runtime-fil (id+whyNow) och register utanför appen; TS-assertioner behålls; mät chunken. |
| genomgang-match-dod-kod | useMatchTimer/useMatchGenerator oimporterade kopior; matchReducer INTERACTIVE_CORNER no-op; RESET_FROM_HALFTIME nollar utvisningar medan generatorn får verkliga; Midfielder+Half båda i enum med olika behandling | rapporterad | Code | §9 | Inventera importerare (LESSONS #34), radera eller ena. Avgör Midfielder-enumens status via grep i generatorerna. |
...
| genomgang-matchprofile-halvlek | Andra halvlek/regenerering rullar ny matchProfile från annan seed — defensive_battle kan bli chaotic i paus, tvärtemot kommentaren i matchCore | in_progress | Codex | §11 | CLAIM 2026-09-12T09:40:51+0200. Skicka `matchProfile` från första halvlekens steg som input i stället för att rulla om. Liten diff, motortest och separat stressmätning efter utvisningsfixens sparade 8,98-baslinje. |
| genomgang-motor-utvisning-halvlek | Utvisningstimers seedas aldrig över halvleks-/regen-gränsen: en utvisning aktiv vid halvtid varar hela 2H + förlängning (0.65/1.20-faktorerna i 30 steg i stället för ≤7). Träffar grovt 35–40 % av matcherna, inbakat i kalibreringen | in_progress | Codex→Opus | §12 | IMPLEMENTERAD 2026-09-12: exakt återstående timer bärs över halvtid, regenerering och förlängning; pausen äter noll tid, legacy count-only får ändlig 10-minuterstimer. 30/30 fokustester, TypeScript och build/fem grindar gröna. Stress 6 843 matcher: grundserie 9,39→8,98 mål/match (−0,41; fortsatt inom 9,12-målet), utvisningar 3,88 mot 3,77. Över avtalad 0,15-rapportgräns: GOAL_RATE_MOD orörd, väntar Opus kalibreringsdom före arkivering. Helsvit 5 211/5 217; sex röda kommer från samtidigt ocommittat O13/rollover-arbete och reproduceras isolerat utanför denna diff. Ursprunglig CLAIM 2026-09-12T09:27:53+0200. |
