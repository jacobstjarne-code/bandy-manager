...
| pending-screen-kedja-stannar | Direkt kedja mellan två obligatoriska pending-skärmar kan stanna på dashboarden efter den första skärmen | verifierad | — | Grind 2-återprov 2026-09-11 + AppRouter.tsx:80–102 | Naturligt återprov reproducerade `qf_summary` → `season_summary`. Kodläsning bekräftar rotorsaken: `DashboardOrPortal` lagrar omdirigering som en boolesk ref och återställer den bara när attention lämnar `screen`; en direkt identitetsväxling mellan två skärmar blockeras därför. Byt till skärmid-buren vakt och regressionstesta direkt skärmkedja. |
