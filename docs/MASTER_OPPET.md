...
| b1-matchhall-omojligt-sasongsmal | Sommaren kan erbjuda ”Få Matchhall färdig” innan hallprövningen kan öppna; samma omöjliga mål kan återkomma och döms som misslyckat i årsboken. Den inaktiva hallraden visar dessutom en falsk chevron. | in_progress | Codex | `docs/playtest/INT_1_B1_LANGBAGE_2026-09-10.md` | CLAIM 2026-09-10T10:15:43+02:00 — Codex. Verifierat mot f40cc862 och filhistorik: `getFacilityGoalOffer` läser nodens generella `available`, medan `shouldStartHallTrial` kräver fullt vanligt anläggningsträd. Om bekräftat, låt målurvalet använda hallprocessens verkliga startbarhet och låt `FacilityTree` ensam rendera chevron när raden är klickbar. Testa negativt med bara Läktare östra byggd och positivt med ett verkligt genomförbart projekt.  Återanvänd getPreSeasonChoices för vanliga startbara noder; Matchhall erbjuds först som verkligt aktivt byggprojekt, inte som obehandlad prövning. |
...
| tranarmarknad-ny-klubb-andra-aret | Första styrelsemötet efter klubbbyte använder andraårs-copy (”Andra året”, ”Första året var …”) trots att managern är på sin första dag i den nya klubben. | rapporterad | Opus/Code | `docs/playtest/INT_1_B1_LANGBAGE_2026-09-10.md` | Verifiera resolvervägen mot aktuell HEAD. Kodläsningen pekar på att `switchManagedClub` korrekt sätter `seasonsAtClub: 1`, men att `resolveBoardMeetingState` väljer A när den nya klubben saknar målhistorik och hela A-poolen är skriven för år två. Opus låser ett sanningsenligt nytt-klubb-tillstånd/copy; Code bygger först efter domen, utan bryggrad i A-poolen. |
