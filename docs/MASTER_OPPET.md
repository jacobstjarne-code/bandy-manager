...
| id | beskrivning | status | ägare | källa | nästa-åtgärd |
|---|---|---|---|---|---|
| decisioncards-likriktning | `DecisionCards`-komponentens visuella likriktning (alla korts enhetlighet, L4) är inte adresserad — bara den ena konkreta luckan | verifierad | Design | BACKLOG.md:74 | verifiera mot kod [Verifierat 2026-09-01 (Code): consequenceLevel/costLabel/irreversible finns nu i två event (economicCrisisService.ts:162, communityRenewalService.ts:290) men DecisionChoices.tsx:s bredare visuella likriktning är fortfarande oadresserad.] Ägare Opus→Design 2026-09-04: visuell likriktning är Designs. Hör ihop med `design-d4-primary-utspadd` (knappvikt) och `design-p4-brytpunkt-knappar` — ta i ett svep. **→ GO 2026-09-08 (Jacob): `design-system/briefs/DESIGN-BRIEF-BESLUTSKORT-2026-09-08.md` (konsoliderar alla tre). Design ritar NU. Ägare Design.** **GPT-KONSOLIDERING 2026-09-08 (`docs/incoming/Beslutskort-brief.dc.html`, källpekad mot `CODE_DECISION_CARD_KONSOLIDERING_2026-06-20` + `DecisionCard.tsx`/`DecisionChoices.tsx`/`SceneChoiceButton.tsx`): de tre raderna är EN primär-regel med tre JACOB-BESLUT — (a) NÄR kopparknappen visas (symmetriska val → all-outline är löst; koppar på asymmetriska = öppet), (b) KNAPPSPRÅK (likrikta `.btn` och `SceneChoiceButton`, eller behåll Georgia-pilknappen som medvetet ceremoni-undantag), (c) PRIMÄR-RANGORDNING (Taktik: vilken av två primärer vinner → taktik-briefen uppgift 2; Portal: kort-rangordningen → CI-R2 `primary-event-vs-farewell`). Design mockar optionerna; Jacob picker de tre. Instruktion i öppet.** **→ BESLUT FÄLLT 2026-09-08 (Jacob): `DOM_BESLUTSKORT_PRIMARREGEL_2026-09-08`. (a) koppar bara på asymmetriska val med reell konsekvensskillnad, annars all-outline. (b) `.btn` överallt; Georgia-pilen enda ceremoni-undantaget. (c) en primär per yta — Taktik-primären i taktik-briefen; Portal-rangordningen är REDAN i `initCardBag`-vikterna (SM-final 100 > deadline 90, avsiktligt), R2-diffarna var reseed-artefakter, ingen ny dom. Design mockar a+b, Code wirar alla tre. Byggbar efter mock.** (Artefakt `Beslutskort-brief.dc.html` konsumerad + arkiverad till `_arkiv-2026-09` 2026-09-08.) |
| taktik-autofyll-knappar-trafyta | Taktiktavlans tre autofyll-knappar ("Starkast"/"Mest utvilad"/"Bäst för dagens match") är 35px höga — under 44px-tumträffgolvet | verifierad | Design | tests/visual/mobileDecisionHierarchy.visual.ts (`findControlSizeViolations`, taktik-testet) | [Verifierat 2026-09-10 (Code, m5-sub-pixel-passet): `git log` visar `minHeight: 35` i `FormationView.tsx` introducerad i `0fb32422` (tidigare session), oförändrad av m5-passet. Testet failar konsekvent: "Starkast" 109×35px, "Mest utvilad" 109×35px, "Bäst för dagens match" 107×35px — alla under 44×44 min. Orelaterat till m5 (textstorlek); egen kontrollstorleks-regression. Ingen fix försökt — hittad som sidoeffekt, inte adresserad.] Kräver ägare: antingen höj knapphöjden (layoutrisk i en redan tät 3-knappsrad, samma bredd-utmaning som m5:s "Fyll laget automatiskt"-rad) eller en dokumenterad `[data-...-exempt]`-motsvarighet om 44px genuint inte får plats. Design/Jacob avgör om det är en Code-mekanisk höjning eller en designrunda. |
...
| transfers-modaler-ledgervokabular | BidModal/RenewContractModal ska få ledger-vokabulär (designidé, öppen fråga) | verifierad | Opus/Jacob | BACKLOG.md:418, 419 | vänta Jacobs beslut [Verifierat 2026-09-01 (Code): docs/BACKLOG.md:418 listar "#7 transfers-modaler (BidModal/RenewContractModal) → ledger-vokabulär (designidé, öppen fråga Opus/Jacob)" som ej löst.] **→ 2026-09-09 (Jacob): INTE parkerad — vill veta mer. Opus utredde (kodläst BidModal/RenewContractModal): modalerna är rent transaktionella (budget, marknadsvärde, lön, år, kontraktstermer, en rivalitetsvarning) och rör ALDRIG liggaren/minnet som nu definierar spelet. 'Ledger-vokabulär' = yta spelarens minnesstånd i själva transaktionen — att lägga bud på en klackfavorit/legend eller förlänga en som bär en transfer_story ska bära minnets vikt, inte bara siffror. Verklig design-idé (Jacobs instinkt stämmer: liggaren är stor, att den inte rört transaktionsspråket är en glugg), men ingen tydlig lucka — blir en Design-brief om Jacob vill ta den. Ägare Opus/Design.** |
...
| sparningsbyte-konflikt-efter-reload | Grindtest 2 kan inte öppnas från Mina karriärer: versionskonflikt återkommer efter reload. | in_progress | Codex | Browser 2026-09-10, port 5173 /saves; Grind 2/3 | Sparningslistan visar Västanfors 2030/31, nästa hemma mot Skutskär. Två öppningsförsök med reload emellan stoppas av ”En annan flik har sparat en nyare version av den här karriären. Ladda om för att fortsätta säkert.” Ingen save har skrivits över eller importerats. Verifiera om konflikten gäller från- eller till-sparningen och om någon annan aktiv flik faktiskt skriver; behåll konfliktskyddet. Blockerar fortsatt långkarriärprov, inte bevis för korrupt data.  CLAIM 2026-09-10T10:47:49+02:00 — Codex. Verifierat mot 50077d3a och git-historik: switchToSave anropar saveSaveGame direkt och sätter bara lastSaveError; persistGameSnapshot sätter däremot saveConflict som öppnar globala SaveConflictModal/resolveSaveConflict. Vanlig reload läser stale Zustand-cache, dialogens action hämtar CAS-kopian. Återanvänd helpern, testa avvisat byte + explicit återhämtning + bevarad senaste data. |
| tranarmarknad-ny-klubb-andra-aret | Första styrelsemötet efter klubbbyte använder andraårs-copy (”Andra året”, ”Första året var …”) trots att managern är på sin första dag i den nya klubben. | in_progress | Code | `docs/playtest/INT_1_B1_LANGBAGE_2026-09-10.md` + `DOM_STYRELSEMOTE_NY_KLUBB_2026-09-10.md` | **→ CLAIM 2026-09-10T10:35:00+02:00 — Code.** [Verifierat 2026-09-10 (Code): domens diagnos stämmer mot HEAD. `resolveBoardMeetingState` (boardMeetingStateResolver.ts:87) väljer state A när `seasonSummaries.length<=1 || fulfillmentPct<0` — `switchManagedClub.ts` nollställer `boardObjectiveHistory: []` vid klubbyte, så `fulfillmentPct` blir -1 och triggar A-grenen ÄVEN mitt i en lång karriär. `advanceProfileToNewClub` (switchManagedClub.ts:65-98) sätter redan `seasonsAtClub: 1` och stänger föregående spell med `toSeason` innan den nya öppnas utan `toSeason` — `clubSpells.some(sp => sp.toSeason !== undefined)` är alltså en robust "har tidigare avslutad spell"-detektor, skiljer korrekt karriärens allra första klubb (ingen stängd spell än) från ett byte. Ingen `manager_appointed`/`club_joined`-liggartyp fanns (bara `manager_return` för återkomst) — tillagd i Narrative.ts.] Bygger: `'N'` i BoardMeetingState + låst copy-pool + GOAL_MOTIVATIONS (ordagrant ur domen), N-gren FÖRE A/B/C i resolvern, `manager_appointed`-liggarpost i switchManagedClub.ts, regressionstest (klubbyte mitt i karriären→N, aldrig A) + negativt test (karriärens första klubb, säsong 2→fortsatt A). **Övertagande-raden (dynamisk sub-rad) SKJUTS UPP:** domen ber Code formulera den generiskt, men det är svensk spelartext — Code skriver aldrig sådan (CLAUDE.md, hård regel). Domen själv erbjuder alternativet: Opus låser en mall separat. Kärnfixet (fel ton i hela mötet) är löst helt utan raden; UI:t renderar bara inte den dynamiska sub-raden än. |
