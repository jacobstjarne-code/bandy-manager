...
**5 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `genomgang-motor-smafynd` stängdes (5→4, se ARKIV) och `bild-slots-tre-scener` (Design-godkänd illustrationsdom, tidigare ouppmärkt lokalt) lades in (4→5).
