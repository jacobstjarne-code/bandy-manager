...
**3 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändringar: `genomgang-save-storlek-matrad` stängdes efter färsk tiiosäsongsmätning (5→4), därefter stängdes `inv-5-fas5-portrattgenerator` när young-poolen blev komplett 1–30 (4→3).
...
| bild-slots-tre-scener | Illustrationsslot i tre Grind 2-utpekade scener (burnout, klack, gala). Låg prio, post-launch, bakom öppen bildpipeline | in_progress | Codex | Domen.dc.html (Design 2026-09-12), godkänd av Opus | CLAIM 2026-09-12T12:56:03+02:00 — Codex. **DESIGNDOM 2026-09-12, godkänd av Opus — 2 ja, 1 nej, 2 rekommenderade slots.** (1) **Burnout: JA, `IllustrationScene mode="band" name="burnout-ceiling"`** — bädda BurnoutMark:s phasemark-div; tyngd i övre halvan, citat/helper/orsaksrad skyddade i fadezonen. Motiv: tömt omklädningsrum sent, en kvarglömd tröja, ingen människa i bild. (2) **Klack: JA, två `mode="header"`-slots, name `klack-tifo` (dag 6) / `klack-konflikt` (dag 11)** — skilda producenter (supporter_tifo/supporter_conflict), samma mode så de läser som en fortsättning, motivskiftet (byggt→splittrat) bär berättelsen. **CODE-VERIFIERING FÖRE WIRING:** bekräfta att de två eventen faktiskt renderas genom en komponent med en egen yta att bädda i — Grind 2 såg dem som inline feed-rader (dag 6/11), och en header-slot på ett inline-kort kan kräva en expanderad vy först. (3) **Gala: NEJ, ingen slot** — GalaScene:s guld-disciplin (medaljen enda fyllda guldytan, kodläst i GalaScene.tsx) får inte lösas upp av en andra ljuspunkt; den typografiska scenen är komplett. Alt om pipeline-symmetri någonsin kräver det: enbart `header` ovanför gala-scene__header, aldrig i guldzonen — men rekommendationen är nej. **BLOCKERARE:** bildpipelinen (Jacob äger; Nano-maneret gav dåligt resultat) — slots kan wire:as tomma nu, placeholder håller dimensionerna, men fylls inte förrän pipelinen är löst. Ej releaseblockerande, post-launch. **KLACK-WIRINGEN VERIFIERAD 2026-09-12 (Code) — Grind 2 hade rätt, det är inline feed-rader.** Spårat hela vägen: `supporter_tifo`/`supporter_conflict` (typ `supporterEvent`) sätter aldrig ett eget `priority`-fält, så `getEventPriority()` (`GameEvent.ts:312`) default:ar dem till `'low'`. `getEventRenderTarget()` (`eventQueueService.ts:79-82`) rutar allt icke-kritiskt/icke-ambient till `'inline'`. `PortalEventSlot.tsx` renderar `'inline'`-mål genom `EventCardInline.tsx` — en 231-raders ren text-/valkortskomponent, noll `<img>`, ingen `IllustrationScene`-import, ingen bildyta alls. Den enda platsen `IllustrationScene` faktiskt är kopplad för events är `EventOverlay.tsx` (via `getEventIllustrationName()`), som bara triggar för `priority === 'critical'`-typer (mecenatEvent, economicStress, playerUnhappy, criticalEconomy) — `supporterEvent` är inte med i den listan och finns ingenstans i overlay-vägen. **Slutsats:** ingen "expanderad vy" existerar att bädda i — `EventCardInline.tsx` ÄR den slutgiltiga renderade formen, bara utan bildstöd. Att ge klacken en header-illustration kräver antingen (a) en ny illustrationsyta i `EventCardInline.tsx` (samma mönster som `getEventIllustrationName`, men kopplad till inline-kortet i stället för overlayn), eller (b) att höja dessa två events prioritet till `'critical'` så de rutas om till `EventOverlay` — vilket ändrar hur de presenteras för spelaren (blockerande modal i stället för inline-notis) och är ett designbeslut, inte ren wiring. **Ingen wiring gjord** — frågan var att svara, inte bygga. Bilderna finns ändå inte än. |
