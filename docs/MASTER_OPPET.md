...
| arsbok-toppbetyg-matchgolv | Årsbokens toppbetyg kan räknas ur en komprimerad restmängd matcher när vardagsmatcher tömts men derbyn/storsegrar ligger kvar; då riskerar årsboken att presentera ett missvisande toppbetyg | in_progress | Code | Brett tvåsäsongsprov / Jacobs order 2026-09-12 | CLAIM 2026-09-12T10:29:00+02:00 — Code. **JACOBS DOM 2026-09-12: HÖJ GOLVET, VIKTA INTE.** Kodläst mot main: `clubFixtures` är färdigspelade ligamatcher för den hanterade klubben och `topRatedEntry` har fortfarande det fasta golvet `r.games >= 5`. **Byt till `>= Math.max(5, spelade_ligaomgångar / 2)`** — elva matcher i en full 22-omgångarssäsong, korrekt degradering i en avbruten. Spelade ligaomgångar = `clubFixtures.length`. Vikta INTE snittet. Riktat test: nio-matchers-spelare får inte slå en 22-matchers-spelare i full säsong. |
