...
**12 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `genomgang-match-dod-kod` stängdes efter rensning och positionsregression, 13→12.
...
| genomgang-toolchain-bump | Vite ^5.2, vitest ^1.4, @types/node 20 — två majors bakom, lock-filen måste följa | in_progress | Codex | §4 | CLAIM 2026-09-12T10:02:00+02:00 — Codex. VERIFIERAD 2026-09-12: `package.json`/lock och installerade versioner bekräftade Vite 5.4.21, Vitest 1.6.1 och @types/node 20.19.37 före ändring. Isolerad kandidat har en commit per steg, grön build och ett deterministiskt 10×5-säsongstest som är byte-identiskt före/efter bortsett från tidsstämpeln. Nästa: landa de tre redan avgränsade committerna. |
| arsbok-toppbetyg-matchgolv | Årsbokens toppbetyg kan räknas ur en komprimerad restmängd matcher när vardagsmatcher tömts men derbyn/storsegrar ligger kvar; då riskerar årsboken att presentera ett missvisande toppbetyg | verifierad | Code | Brett tvåsäsongsprov / Jacobs order 2026-09-12 | **JACOBS DOM 2026-09-12: HÖJ GOLVET, VIKTA INTE.** Opus kodläst seasonSummaryService.ts: computeSeasonRatings + topRatedEntry har redan ett golv `r.games >= 5`, men fem är för lågt för en 22-omgångarssäsong (Nils Hård vann på nio). **Byt `>= 5` till `>= Math.max(5, spelade_ligaomgångar / 2)`** i topRatedEntry-filtret — kräver elva matcher i en full säsong, degraderar korrekt i en avbruten. Spelade ligaomgångar = clubFixtures.length (redan beräknat i generateSeasonSummary). Vikta INTE snittet mot speltid (gör siffran oläsbar; årsboken ska visa sant snittbetyg). En rad + ett riktat test (nio-matchers-spelare vinner inte över 22-matchers i full säsong). Låg prio, ej releaseblockerande. |
| genomgang-save-storlek-matrad | TS-8/GAP-5 (SaveGame-tillväxt, eventLedger utan tak) är oro utan mått | rapporterad | Code | §5 | DEV-loggrad i persistAutosave; läs av efter Codex 8-säsongskarriär; < 2 MB → stäng, annars spec. |
| genomgang-jacob-beslut | (a) bandygrytan-datan i repot eller gitignorad — **AVGJORD 2026-09-12: STANNAR I REPOT.** Jacobs dom: icke-problem i praktiken, ingen persondata; databasskydds-risken vägs inte tyngre än friktionen att flytta. Ingen åtgärd. (b) mätbart soft-launch-mål — **AVGJORD 2026-09-12, se release-definitionen** | verifierad | Code | §Jacob-beslut | (a) INGEN ÅTGÄRD. (b) Opus skriver in de två målen i RELEASE_DEFINITION_MJUKLANSERING.md (season_completed ≥ 1 för båda testarna + median session_end.durationSeconds > 15 min, inom två veckor). Ingen Code-åtgärd förrän analytics-avläsning efter soft-launch. |
| genomgang-motor-attribut-aldras-ej | Hanterade klubbens spelare får ALDRIG attributnedgång med ålder (developPlayers är AI-only; applyRoundDevelopment rör bara CA; motorn läser attribut). AI-veteraner sjunker, dina inte. | in_progress | Code | §7 | CLAIM 2026-09-12T14:00:00+0200 — Code. **JACOBS DOM 2026-09-12: BYGG NEDGÅNGEN (är inte balans, är ett trasigt system).** Opus kodläst playerDevelopmentService.ts: developPlayers har hela nedgångsfasen (negativ getAgeFactor från 31, attributnedgång på alla attribut) men körs bara på AI. applyRoundDevelopment (managed, per omgång) rör bara currentAbility, lämnar attributes orörda; evaluateSquad läser attributen — så din 35-åring behåller skating:88 för evigt. **Bygg `applyVeteranAttributeDecline(players, managedClubId, seed)`**: körs vid säsongsslut (seasonEndProcessor) på managed-spelare med age>=31, applicerar developPlayers NEDGÅNGSGREN (getAgeFactor<0-vägen, samma baseChange-formel, samma random-extra-decline-attribut) på `attributes` — men RÖR INTE currentAbility (omgångssystemet äger CA; annars dubbelräknas nedgången). Återanvänd getAgeFactor/getArchetypeMultiplier, gissa ingen ny konstant. Mät med stress 6 säsonger: managed-veteranernas attributsnitt ska konvergera mot AI-veteranernas kurva (idag: platt). Riktat test: en 34-åring i managed vs AI med identisk startattribut ska ha jämförbar nedgång efter 3 säsonger. Releaserelevant — ändrar långtidsbalansen i truppbygget. |
| genomgang-portal-effekter | PortalScreen: ResizeObserver-effekt utan deps; mount-advance dubbelkörs i StrictMode; body.style-mutation från route; CTA-etikettlogik i vyn; BottomNav rå rgba | rapporterad | Code | §10 | Fem små edits, browserprov på 390 px. |
| genomgang-motor-utvisning-halvlek | Utvisningstimers seedas aldrig över halvleks-/regen-gränsen: en utvisning aktiv vid halvtid varar hela 2H + förlängning (0.65/1.20-faktorerna i 30 steg i stället för ≤7). Träffar grovt 35–40 % av matcherna, inbakat i kalibreringen | in_progress | Codex | §12 | **OPUS KALIBRERINGSDOM 2026-09-12: glidningen STÅR, GOAL_RATE_MOD rörs INTE.** Skäl: 9,12-målet mättes med den trasiga eviga-powerplay-mekaniken inbakad — det var motorns snitt MED en målgödande bugg, inte dess sanna snitt. Att kompensera tillbaka till 9,12 vore att återinföra buggens aggregateffekt efter att ha tagit bort dess orsak (LESSONS: kalibrera mot verkligheten, inte mot ett tal). 8,98 mot verkligt 9,12 är 1,5 % lågt och ärligt; GOAL_RATE_MOD är dessutom fel spak (global, träffar alla fem målvägar i alla matcher, medan underskottet bara finns i 2H i matcher med utvisning). ENDA kvarvarande villkor före arkivering: kör `npm run calibrate:firing:formal` (10 000 seeds, 6 säsonger) och bekräfta att UTVISNINGSANTALET håller 3,77/match — den siffra SUSPENSION_FREQUENCY_MOD faktiskt kalibrerades mot. Rapportens 3,88 (6 843 matcher) är +2,9 %, inom brus men värt en formell körning. Håller antalet ≤3,85: arkivera, ingen konstantändring. Glidit >3,9: justera SUSPENSION_FREQUENCY_MOD (inte GOAL_RATE_MOD), eftersom det då är frekvensen som rört sig. Ta bort dubbla ENGINE_VERSION i samma commit (genomgang-motor-smafynd §13 nämner den också — vem som än rör matchCore först). — IMPLEMENTERAD 2026-09-12: exakt återstående timer bärs över halvtid, regenerering och förlängning; pausen äter noll tid, legacy count-only får ändlig 10-minuterstimer. 30/30 fokustester, TypeScript och build/fem grindar gröna. Stress 6 843 matcher: grundserie 9,39→8,98 mål/match (−0,41), utvisningar 3,88 mot 3,77. Helsvit 5 211/5 217; sex röda kommer från samtidigt ocommittat O13/rollover-arbete och reproduceras isolerat utanför denna diff. Ursprunglig CLAIM 2026-09-12T09:27:53+0200. |
| genomgang-motor-smafynd | penaltiesHome/Away hårdkodade 0 i matchEngine; fatigueLevel = 100−morale; två ENGINE_VERSION; keeperDive hårdkodad 'offensive'; AI-ungdomsintag åldras +1 och får historik vid skapande; freeAgents är kopior som inte åldras (roten till dubbelposten Opus täppte i signFreeAgent) | rapporterad | Code | §13 | En commit, sex punkter, riktade tester. freeAgents-roten är störst — gör den till id-lista/projektion. |
| genomgang-portal-cue-typografi | Portalens "Vad nu?"-cue är 10,5 px inline — under typografikanons minsta roll (h-quote-sm 11 px) | rapporterad | Opus→Design | §14 | Design väljer textroll, Code byter. |
