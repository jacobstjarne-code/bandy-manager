...
**10 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `genomgang-jacob-beslut` stängdes efter att båda besluten förts till sina kanoniska dokument, 11→10.
...
| genomgang-portal-effekter | PortalScreen: ResizeObserver-effekt utan deps; mount-advance dubbelkörs i StrictMode; body.style-mutation från route; CTA-etikettlogik i vyn; BottomNav rå rgba | in_progress | Code | §10 | CLAIM 2026-09-12T10:37:00+02:00 — Code. VERIFIERAD mot kod och git: samtliga fem fynd kvarstår. Observatören saknar deps, mount-effekten saknar refvakt, PortalScreen muterar `document.body`, CTA-etikettens cup/slutspel/åskådardom ligger inline och BottomNav duplicerar den globala `texture-wood`-gradienten med rå rgba. Bygg beteendebevarande: refvakt, observer-livscykel, GameShell-ägd portalbakgrund, ren CTA-funktion i `nextActionCue.ts` med tester och återbrukad trätexture; browserprov 390 px. |
