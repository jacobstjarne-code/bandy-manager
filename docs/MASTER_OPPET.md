...
**13 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: den redan levererade dubbletten `genomgang-contentcontract-bundle` stängdes, 14→13.
...
| genomgang-match-dod-kod | useMatchTimer/useMatchGenerator oimporterade kopior; matchReducer INTERACTIVE_CORNER no-op; RESET_FROM_HALFTIME nollar utvisningar medan generatorn får verkliga; Midfielder+Half båda i enum med olika behandling | in_progress | Codex | §9 | CLAIM 2026-09-12T10:11:28+0200 — Codex. VERIFIERAD mot kod och git: hooks har noll importerare; INTERACTIVE_CORNER har noll avsändare; RESET-utvisningarna är redan rättade i `1b0528e3`; Half/Midfielder är två riktiga roller i Formation/motor/UI men `loadGame` kollapsar felaktigt alla Midfielder till Half. Rensa döda kopior/no-op och ta bort den överspelade laddningsmigreringen, med regressioner. |
