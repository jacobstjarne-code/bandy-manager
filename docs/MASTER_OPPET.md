...
| inv-4-o12-forhandsdelta | O12 "förhandsdelta" (DOM_DOMINANS_OCH_FORHANDSDELTAN) skriven men aldrig byggd | in_progress | Codex | INVENTERING_2026-08-31.md:103 + SLUTTEST_KO.md:1060 | CLAIM 2026-09-09T13:14:27+02:00 — Codex. Bygger browserrapportens mekaniska sanningsdel: varje resolution får en stabil egen identitet och `ResolvedChoice` skiljer genuint flerval från enknappskvittens, så entropin varken räknar `Noterat` eller tappar transferkedjans slutsvar. Browserprovet är genomfört och rapporterat i `RAPPORT_O12_VALENTROPI_BROWSER_2026-09-09.md`: 3 nya mobilkarriärer, 3 exportsaves, 15 explicit spelarval. Riktiga flervalsrader gick att mäta, men provet är avsiktligt varierat och kan därför inte godkänna den populationsbaserade 80 %-grinden. Två rotfel verifierades: (1) `voiceIntroductionService` återanvänder `journalistExclusive`/`supporterEvent` för enknapps-`Noterat`, och `ResolvedChoice` saknar beslut-vs-kvittens så entropin får falska 100 %-rader; (2) transfermotbud+slutsvar delar event-id och `(save.id,eventId)`-dedup tappar ett verkligt steg. Dessutom syntes O12:s ursprungsfel i riktig UI: exakta icke-pengadeltan före val (`+2/+4/+5 moral` i presskonferensen). Efter den mekaniska delen återstår Opus/Code-delningen av `EventChoice` i riktad förhandsbeskrivning och exakt efterkvitto. Ingen eventtyp-blacklist och ingen regex-suddning — det vore bryggfixar. D1:s ambientväg och O5 finns redan. Dedup 2026-09-03: konsoliderar `sluttest-o12-forhandsdeltan` (samma punkt, två källor). |
...
| sluttest-o10-bestinclass | O10 best-in-class-strategin: `BESLUTAD, EJ PÅBÖRJAD` (bandyarkivet, vägskäl, bruksligor, utmaningslänkar, skaparekosystem) | verifierad | Jacob | SLUTTEST_KO.md:1050 | → 2026-09-08 (Jacob): seed-i-länk-skivan byggs nu (se `o10-queryparam-clubselection`). RESTEN (bandyarkivet, vägskäl, bruksligor, skaparekosystem) PARKERAD post-launch, dokumenterad i `POST_LAUNCH.md`. |
...
| sluttest-grind4 | Grind 4 (tillväxten: riktiga mottagare startar karriär från spelarlänk) ej dokumenterad som passerad | verifierad | GPT | SLUTTEST_KO.md:380 + TESTINSTRUKTION_GRIND_234_2026-09-08.md | BLOCKERAD på `o10-queryparam-clubselection`. När O10 är byggd: starta från riktig delad länk och verifiera seed, klubb och regelversion; ingen påhittad testväg. |
