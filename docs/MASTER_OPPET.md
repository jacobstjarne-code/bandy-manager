...
| stickiness-dataskydd | Servern lagrar installation-id, save-id, attention-snapshot (open loops, kandidater, state-version) och leveranshistorik per installation — en ny datakategori för spelet, som hittills varit local-first utan konto | in_progress | Codex | Implementation "Återstår" 8 | CLAIM 2026-09-10T16:08:27+02:00 — Codex. Jacob har godkänt policyn: omedelbar full radering när notiser stängs av, automatisk full radering efter 90 dagars inaktivitet och aldrig namn, e-post eller hel sparfil på servern. Avregistrering och integritetstext finns redan; bygger den saknade automatiska gallringen mot samma installationsgräns. Snapshotens open loops är spelarens beslutsläge och ska behandlas så. |
