...
| id | beskrivning | status | ägare | källa | nästa-åtgärd |
|---|---|---|---|---|---|
| c-o1sp1-kontextuella-sponsorer | O1:s sponsorkonflikt är 4/5 av varsel-mallen; kontextuella sponsorer med stabila namn är onåbara som rival p.g.a. kategori-namnrymd | in_progress | Codex | BACKLOG.md:541 | CLAIM 2026-09-09T06:18:47+02:00 — Codex. OPUS-DOM-FÖRST (ej blint): samma rot som stickiness-copy `categoryFor()` — kategori används som namnrymd och konflaterar IDENTITET (vem) med ROLL/KATEGORI (vad slags), så en namngiven kontextuell sponsor kan inte också adresseras som rival/antagonist. Riktning (samma generalisering som R3/ålderskurva/beslutsbyggarna): skilj stabilt id från roll-medlemskap — en entitet har ETT id och kan hålla FLERA roller (sponsor OCH antagonist), i stället för att rollen är dess identitet. **INTE samma rot som stickiness `categoryFor()`** (grundad läsning 2026-09-06 falsifierade den tidigare hopkopplingen — `categoryFor` handlar om NOTIS-kategorier klassade från fel källa, inte subjekt-identitet; se `stickiness-categoryfor-tre-kallor`). Denna rör subjekt-namnrymden: kräver läsning av hur sponsor/rival/subjekt keyas (player/club/mecenat) innan kontrakt. Ägare Opus, dom pending grundad läsning — står kvar som egen fråga, ej blockerad av eller på categoryFor. |
