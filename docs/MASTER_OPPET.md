...
| id | beskrivning | status | ägare | källa | nästa-åtgärd |
|---|---|---|---|---|---|
| statistik-telemetri-ror | Statistik & tracing: tratt (onboarding→säsong N), retention, hur de spelar — anonymt, opt-out, gallring | in_progress | Codex | Jacobs fråga 2026-09-10 + `postgresStore.js` (kodläst) | CLAIM 2026-09-10T08:45:00+02:00 — Codex. **SPEC KLAR: `SPEC_TELEMETRI_STATISTIK_2026-09-10.md`.** Egen `analytics_events`-tabell (barn till installationen, `ON DELETE CASCADE`), skild från `attention_events` så push-svarsprofilen inte grumlas. Sju event: `install`, `game_created` {klubb,svårighet}, `onboarding_done`, `first_match`, `season_completed` {säsong,placering}, `game_over` {skäl,överlevda säsonger}, `session_start/end`. Hur de VÄLJER = O12-beslutstelemetrin, dupliceras inte. Opt-out i `preferences`-jsonb + toggel i Inställningar (Jacobs beslut); klienten slutar skicka, servern släpper ändå opt-out:ade (försvar på djupet). Gallring 90 dygn på `recorded_at`. Hållning: diagnostisk, inte engagemangs-maximerande. Inställningstext (utkast) i specen, låses på Jacobs nick. **→ 2026-09-10 (Jacob): skickat till Codex.** Röret (tabell + händelsesändare + gallringsjobb) byggs i V1 medan backend-koden är öppen; dashboard/läs-vy är post-launch. Ägare Codex (bygge) + Opus (inställningstext). |
