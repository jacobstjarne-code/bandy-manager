...
| int-1-stora-bagarna | 'Bygg B1' är i huvudsak utförd som körorder; kvarstår är Jacobs genomspelning/känsla, inte mer kod | verifierad | Jacob | BACKLOG.md:659 | B1-substratet till stor del byggt (FacilityScreen, O5/O17, framgångskurvan stängd, tränarmarknad, årsbok alla fem fält) **→ 2026-09-09 (Jacob): omformad till SPELTEST-rad för Codex. Substratet ÄR byggt; kvar är en genomspelning/känsla-check, inte kod. Codex kör den på egen hand i sin pågående långkarriär — spela B1-bågen (anläggning → framgångskurva → tränarmarknad → årsbok), rapportera om den känns hel eller har en glugg. Ägare Codex (speltest).** |
...
| sluttest-grind2 | Grind 2 (andra akten: ekonomiskt val år åtta, ingen upprepad pivotal scen, färdigt anläggningsträd) ej dokumenterad som passerad | in_progress | Codex | SLUTTEST_KO.md:376 + TESTINSTRUKTION_GRIND_234_2026-09-08.md | CLAIM 2026-09-09T08:21:53+02:00 — Codex. Kör samma riktiga minst åtta säsonger långa karriär som Grind 3 och dokumentera konkreta ekonomiska val, pivotal repetition och anläggningsträd i `docs/playtest/`; Opus domar resultatet efter leverans. |
| sluttest-grind3 | Grind 3 (rytmen: primär handling, nästa olösta fråga, en landning per säsong) ej dokumenterad som passerad | in_progress | Codex | SLUTTEST_KO.md:378 + TESTINSTRUKTION_GRIND_234_2026-09-08.md | CLAIM 2026-09-09T08:21:53+02:00 — Codex. Samma långkarriär som Grind 2; för varje säsong dokumenteras primär handling, nästa olösta fråga och landningen i `docs/playtest/`. Grind 4 lämnas blockerad på O10. |
| supporter-konflikt-resolved-dedup | `Konflikt i klacken` återkom med identisk ingress och samma tre val i omgång 10 efter att `Bjud in båda på ett möte med truppen` lösts i omgång 8. | rapporterad | Codex | Grind 2/3, riktig browserkarriär (klientens byggfot `5f4dada3`, lokal HMR), säsong 2030/31, 2026-09-10 | Kodläs producent, resolved-/deferred-dedupe och git-historik; skilj nygenerering från redan sparad kö innan rotorsaken fastställs. **Opus-lead 2026-09-10 (kodläst supporterEvents/eventResolver/deferredRolloverService, EJ git-log):** dedup-guarden FINNS — `resolveEvent` skriver `conflictSeason` OCH pushar id:t till `resolvedEventIds`, `generateSupporterEvents` gejtar på båda; en ren resolution kan inte återskapa eventet. INTE saknad-dedup-klassen. Stödjer 'redan sparad kö'-spåret: konfliktkortet är röstgejtat (`voiceId` klackledaren), lever i `deferredDecisions` tills gaten öppnar (förklarar omg 8→10, inte 9). Hypotes: löses det medan det ligger i `deferredDecisions` utan att först injiceras i `pendingEvents` faller `resolveEvent` på `if (!event) return game` — eventet varken resolveras, tas bort ur kön eller markörsätts, samma sparade instans återkommer när gaten öppnar igen. Rollover-vägen gör redan rätt (`resolveDeferredAtRollover` injicerar temporärt i pendingEvents). Sista milen för Codex: bekräfta i store-resolve-/deferred-promotion-vägen om ett deferrerat `supporter_conflict_` resolveras utan pendingEvents-injektion; om ja → spegla inject-mönstret + regressionstest, flippa till verifierad. |
