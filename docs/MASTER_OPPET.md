...
**5 aktiva poster:** `stickiness-drift-backend`, `begriplighet-klass-h-kartlaggning`, `begriplighet-b-flerommgangshopp-financelog`, `begriplighet-e-flikintroduktioner`, `begriplighet-f-granska-eko-yta`. BEGRIPLIGHETSREVISION_2026-09-12-passet stängde klass A/B/C/D (fullständigt, se ARKIV) 2026-09-15; klass E och F:s granska-eko-delar spanns ut till egna rader eftersom de kräver Codex-struktur respektive Opus-text/ett trådningsbeslut — inte längre Codes jobb just nu. Räkna endast faktiska datarader med status `rapporterad`, `verifierad` eller `in_progress`; historiska räknarändringar och stängda poster finns i `docs/MASTER_ARKIV.md`.
...
| begriplighet-e-flikintroduktioner | Spunnen ur begriplighet-klass-d-e-inventering (BEGRIPLIGHETSREVISION_2026-09-12 §Klass E), nu stängd för D-delen. Orten/Sälj/Fria-flikarna saknar FeatureIntroduction trots att systersystem (Ekonomi/Bygget/Marknad/Scouting) fick det; Tränare-fliken saknar en kort förklarande rad om "belastning" | verifierad | Codex→Opus | §Klass E (Opus-dom 2026-09-15) | Opus-domen finns redan i den arkiverade radens fulltext (MASTER_ARKIV.md, begriplighet-klass-d-e-inventering): **Orten — JA, högst prio** (samma ort/kommun-röst som entré-bågen). **Sälj — JA** (oåterkallelig, ekonomiskt tung, systersystem fick det). **Tränare-fliken — JA men TEXT, inte full introduktion** (kort rad om belastningsmekaniken räcker). **Fria-listan — JA**, samma systersystem-argument som Sälj. **Nu/Minne/Hem/Match/Tabell — NEJ**, självförklarande. **Ordning (Opus egen):** Codex bygger strukturen (vem äger ytan, när öppnas den, FeatureIntroduction-wiring) FÖRST — Opus skriver rösterna (Orten/Sälj/Fria-introduktionerna + Tränare-raden) i ETT svep när strukturen står, ingen platshållartext under tiden. |
| begriplighet-f-granska-eko-yta | Spunnen ur begriplighet-klass-f-kartlaggning (BEGRIPLIGHETSREVISION_2026-09-12 §Klass F), nu stängd för schema/kafferum-delen. subject.kind==='manager' taggas redan 'review' i redaktorenService.ts:s fitsSurfaces, men ingen yta konsumerar den agendan generiskt | rapporterad | Code→Opus | §Klass F (uppföljning, MASTER_ARKIV.md) | Grep bekräftar: `agendaForSurface(..., 'review')` läses BARA av `reviewCallbackService.ts`s två smala former-player-callbacks (`post.subject?.kind==='player'`+specifika typer, matchresultatberoende). En manager-subject-post kan alltså vara korrekt klassad men aldrig visas. Två vägar: (a) en ny, generisk "senaste granskningsbara post"-sektion i `GranskaOversikt.tsx` (arkitekturbeslut, var i vyn), eller (b) en riktad `selectManagerReviewCallback`-funktion i samma stil som de två befintliga (kräver ny svensk text — Opus). Ingen platshållartext, ingen gissning — kräver ett VAR-beslut innan Code bygger vidare. |
