...
|---|---|---|---|---|---|
| mecenat-silentshout-aterfall | Grind 2 reproducerade `Anna Johansson hotar` igen tre omgångar efter att samma kort lösts. Generatorn tidsstämplar alla silentShout-id:n med `Date.now()`, vilket gör den kanoniska köns dedupe verkningslös. Samma korts submit-underrad lovar dessutom `silentShout ökar`, men den deklarerade effekten ändrar bara `mecenatHappiness`. | rapporterad | Code/Opus | `docs/playtest/GRIND_2_3_LANGKARRIAR_2026-09-09.md`, säsong 5 semifinal 1/4; `mecenatService.ts` | Verifiera hela producentklassen och välj en befintlig kanonisk minneskälla för ”kan återkomma om siffran stiger igen”; skapa ingen ny sekundär logg. Fastställ därefter om den felaktiga underraden kan rättas genom att stryka det obelagda ledet eller kräver ny Opus-text. |
