...
**7 poster kvar här, alla aktiva:** `rapporterad`/`verifierad`/`in_progress`. Ingen `klar`/`stale`-rad ska längre stå kvar i den här filen. **Räkneregel:** räkna bara faktiska datarader (aldrig tabellhuvuden eller avdelningsrubriker), och uppdatera detta tal i samma commit som en post läggs till eller flyttas till arkivet. Historik förs i `MASTER_ARKIV.md`; senaste räknarändring: `genomgang-toolchain-bump` och `genomgang-motor-attribut-aldras-ej` stängdes parallellt, 9→7.
...
| genomgang-motor-smafynd | penaltiesHome/Away hårdkodade 0 i matchEngine; fatigueLevel = 100−morale; två ENGINE_VERSION; keeperDive hårdkodad 'offensive'; AI-ungdomsintag åldras +1 och får historik vid skapande; freeAgents är kopior som inte åldras (roten till dubbelposten Opus täppte i signFreeAgent) | in_progress | Code | §13 | CLAIM 2026-09-12T15:10:00+0200 — Code. En commit, sex punkter, riktade tester. freeAgents-roten är störst — gör den till id-lista/projektion. OBS: matchCore.ts/matchEngine.ts delas med Codex (genomgang-motor-utvisning-halvlek, in_progress) — verifiera aktuellt filläge (ENGINE_VERSION m.m. kan redan vara rört) innan edits, isolera med git add -p vid commit. Kopplat fynd: freeAgents-dubbleringen är roten till att karriar_vastanfors_10sasonger.json (10 säsonger) väger 7,48 MB — transferState.freeAgents ensamt 3,97 MB (1359 poster) av 2,57 MB players. Löser trolen genomgang-save-storlek-matrad-mätningen (>2 MB-tröskeln) på samma gång. |
| genomgang-portal-cue-typografi | Portalens "Vad nu?"-cue är 10,5 px inline — under typografikanons minsta roll (h-quote-sm 11 px) | in_progress | Codex | §14 | CLAIM 2026-09-12T11:01:37+02:00 — Codex. VERIFIERAD mot `PortalScreen.tsx`, `global.css` och införandecommit `0c8c6365`: cuen är handlingshjälp, inte citat eller versal label; byt den hårdkodade 10,5 px-stilen till befintliga `.h-body-sm` (11 px) och behåll tonfärgen som lokal modifierare. |
