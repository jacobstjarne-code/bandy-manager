...
| analytics-dev-scen-sasongshistorik | AnalyticsBridge/analyticsLifecycle kan krascha när en dev-save saknar seasonSummaries. | in_progress | Codex | Speltestjournal 2026-09-10; aktuell 9e2a8af7 har fortsatt oskyddad length/iteration, nya browserförsöket timeoutade. | CLAIM 2026-09-10T09:34:00+02:00 — Codex. Kod/git verifierade på 9e2a8af7: AnalyticsBridge läser game?.seasonSummaries.length, initializeAnalyticsBaseline och syncGameAnalytics itererar utan fallback. DevScenesScreen.makeGame saknar fältet. Skydda konsumenterna mot saknad historik; ingen migration eller ändring av sparningar. Testa tom historik och befintliga riktiga milstolpar. |
| supporter-konflikt-resolved-dedup | `Konflikt i klacken` återkom med identisk ingress och samma tre val i omgång 10 efter att `Bjud in båda på ett möte med truppen` lösts i omgång 8. | in_progress | Codex | Grind 2/3 + kod/git 2026-09-10 | CLAIM 2026-09-10T09:21:26+02:00 — Codex. Kodläst eventResolver, gameStore, PortalQueueRail och decisionBudgetService (senast ca419fb0): UI resolverar pendingEvents, inte direkt deferred. Resolution tar bort pending-kopian och registrerar resolvedEventIds; applyDecisionBudget sammanfogar däremot deferred + pending utan id-dedup eller resolved-filter. En äldre kökopia kan därför återvisas trots korrekt resolution. Reparera centralt vid köpartitionering, behåll olika event-id:n och testa gammal dubblett + löst id. Den ursprungliga browserinstansens exakta köhistorik är inte bevisad. |
