...
| rc-arsbok-komplett-sanningsunderlag | Årsbokens spelarpris och styrelse-/licensdom kan bygga på ofullständigt eller motstridigt underlag | in_progress | Codex | docs/playtest/GRIND2_BRETT_TVASESONGSPROV_ASTRA_27DA71B3_2026-09-11.md | CLAIM 2026-09-11T19:58:04+02:00 — Codex. `stripCompletedFixture` tömmer ratings i vanliga matcher medan `generateSeasonSummary` räknar topRated ur det som råkar finnas kvar; urvalet gynnar derby/storseger. I samma körning var ”mer än de bad om” och ”vad de väntade sig” samtidiga, och ”två underskott” motsade synliga +183/−793 tkr. Frys komplett säsongsaggregat före komprimering och låt domtexterna läsa samma redovisade sanning. |
...
| id | beskrivning | status | ägare | källa | nästa-åtgärd |
|---|---|---|---|---|---|
| inv-4-o12-forhandsdelta | O12 "förhandsdelta" (DOM_DOMINANS_OCH_FORHANDSDELTAN) skriven men aldrig byggd | verifierad | Opus | INVENTERING_2026-08-31.md:103 + SLUTTEST_KO.md:1060 | **→ MEKANISKT DELPASS KLART `319ccd93` (2026-09-11, Codex):** varje nytt valkvitto bär nu en stabil beslutsmallsnyckel (eventtyp + sorterad choice-set, med explicit producent-id som escape hatch), och entropiverktyget grupperar per mall i stället för bred eventfamilj. Äldre kvitton utan säker mallidentitet exkluderas. Patronintroduktionens falska +20/+5-val är ersatt av en ambient enknappskvittens utan state-bonus; introduktions- och röststämplar bevaras. Build grön; riktat 77/77. Helsviten gav 5 187/5 192 på första körningen enbart för att fem exakta schemaassertioner väntade det gamla kvittot; samtliga berörda vägar kördes därefter grönt 26/26. **KVAR:** spelmässig dom för `refereeMeeting` (+1/0/−1) och därefter ny naturlig population på aktuell main; burnout/förnyelse står fortsatt som populationshypoteser. Aktiv räknare fortsatt 8. Tidigare claim: 2026-09-11 18:59 CEST — Codex. §1 KLAR `06336d37`: instanssann telemetri, kvittenser exkluderas och flerstegskedjor bevaras. §2 KLAR `b5e6dfad`: `subtitle` är kvalitativ förhandstext enligt låst dom `DOM_O12_FORHANDSTEXT_KONTRAKT_2026-09-09.md`; exakta pengar är kvar, faktisk clampad icke-penga-diff sparas strukturerat i `ResolvedChoice.outcomeDeltas` och visas först efter valet i Granska. Dolda development/discipline-motvikter läcker inte. Ny statisk bygggrind stoppar regression utan runtime-sanering. Bygge grönt; riktat 111/111; helsvit 541/4 936. Browserbelägg och full redovisning: `RAPPORT_O12_VALENTROPI_BROWSER_2026-09-09.md`. Återstår deployprov och naturlig population innan 80 %-grinden kan dömas; ett medvetet varierat tre-save-prov bevisar bara rören. D1:s ambientväg och O5 finns redan. Dedup 2026-09-03: konsoliderar `sluttest-o12-forhandsdeltan`. **→ §3 KLAR `5692246d` (2026-09-09, Code/Codex): veckobesluten använder nu ett separat kvalitativt `preview` före valet och ett strukturerat, clampat före/efter-kvitto efter resolution; O12-grinden täcker hela `WeeklyDecision`-katalogen. Sju nya regressionstester och full svit gröna. Browserverifiering av den ursprungliga `corner_extra_training`-läckan finns i `RAPPORT_O12_VECKOBESLUT_BROWSER_2026-09-09.md`; skivan är arkiverad som `inv-4b-o12-veckobeslut`. Deployprov + naturligt O12-prov 2026-09-11 är RÖTT; full evidens i `RAPPORT_O12_NATURLIGT_PROV_2026-09-11.md`. Före/efter-kontraktet höll i produktion, men `refereeMeeting` är strikt dominant (+1/0/−1 i samma system; respect 20/23) och patronintroduktionen är +20 mot +5 i samma system. Analysen grupperar dessutom på bred eventtyp och maskerar dominanta mallar inuti `patronEvent`. Beslutsmallsidentitet och patronintroduktion är åtgärdade i `319ccd93`; domarmötet väntar fortfarande riktig motkostnad eller ambient-dom. Burnout/förnyelse står som populationshypoteser. Aktiv räknare fortsatt 8.** |
