...
|---|---|---|---|---|---|
| mecenat-silentshout-aterfall | Grind 2 reproducerade `Anna Johansson hotar` igen tre omgångar efter att samma kort lösts. Generatorn tidsstämplar alla silentShout-id:n med `Date.now()`, vilket gör den kanoniska köns dedupe verkningslös. Samma korts submit-underrad lovar dessutom `silentShout ökar`, men den deklarerade effekten ändrar bara `mecenatHappiness`. | in_progress | Codex | `docs/playtest/GRIND_2_3_LANGKARRIAR_2026-09-09.md`, säsong 5 semifinal 1/4; `mecenatService.ts`; `eventProcessor.ts`; `postAdvanceEvents.ts` | CLAIM 2026-09-10T11:58:00+02:00 — Codex. Kod- och historikverifierat 2026-09-10: alla fyra varianter får nya `Date.now()`-id:n; både `postAdvanceEvents` och `eventProcessor` anropar samma generator, men bara den senare skickar spelarens namn och kan nå transfergrenen. `silentShout` ökas säsongsvis av `updateSilentShout`; den befintliga kanoniska dedupen är `pendingEvents + deferredDecisions + resolvedEventIds`. Bygg med en producent, stabilt id per tröskelvariant och den befintliga dedupen. Stryk bara det obelagda `silentShout ökar` ur submits befintliga underrad; ingen ny copy behövs. |
