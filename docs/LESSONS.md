...
## 50. Ett omöjligt testtillstånd testar inte verkligheten — tredje gången ett grönt test bevisade fel sak

**Mönster (2026-08-27):** vid `standing.position`-läst-före-initiering-fixen (`safeStandingPosition`, PASTAENDEKARTAN) blev tre BEFINTLIGA, tidigare gröna tester röda: `demandEngine.test.ts`s och `eventProcessor.demandEngine.test.ts`s `league_position`-scenarier konstruerade en klubb med `position: 1` (eller `position: totalTeams`) men `played: 0` — ett tillstånd som ALDRIG kan uppstå i en riktig match (en klubb kan inte vara "1:a i tabellen" med noll spelade matcher och riktiga poäng). Testerna var gröna för att funktionen, INNAN fixen, gav rätt svar för just den kombinationen — men kombinationen själv var påhittad, inte en verklig speltillstånd.

**Samma klass, tredje gången:** multiSlotSwitch-incidenten och M9:s `RedCard`-enum-mätning (lärdom-familjen kring #16 och `avgSuspensionsPerMatch`) delade samma rotorsak — ett test som är tekniskt korrekt mot sin EGEN fixtur, men fixturen beskriver ett tillstånd produktionskoden aldrig producerar. Testet bevisar då att funktionen hanterar ETT PÅHITTAT FALL rätt, inte att den hanterar VERKLIGHETEN rätt — och skillnaden syns först när någon annan (en riktig bugg, en ny grind) råkar sätta ljuset på just den kombinationen.

**Rotorsaken:** en testfixtur byggs oftast genom att ta en bas (`createNewGame()` eller motsvarande) och override:a EXAKT det fält testet vill kontrollera (`position: 1`) — utan att fråga om resten av objektets fält fortfarande är internt konsistenta med det nya värdet. `played` glömdes kvar på sitt bas-värde (0, ett färskt spels startläge) medan `position` sattes till ett värde som bara är meningsfullt om `played` också är verkligt.
...
**Rotorsak:** Berättaren/redaktören är inte EN grind utan FLERA separata, typ-nyckla allowlistor (`REVIEW_TYPES`, `PRESS_TYPES`, `STATIC_MOMENT_KIND`, `MOMENT_FAMILY`, `hasMomentViewClaimContract` m.fl.) — varje ny yta en typ ska nå kräver sin egen registrering. `EventLedgerType`-unionen är bara en deklaration av att typen FÅR skrivas; den ger ingen typen automatiskt tillträde till någon yta. Samma familj som #54 (producent utan konsument) och #55 (filter med dolt antagande) — den tredje varianten: producent OCH konsument finns, men den ROUTANDE strukturen mellan dem känner inte igen typen.

**Fix:** när en ny `EventLedgerType` läggs till, sök `grep -rn "REVIEW_TYPES\|PRESS_TYPES\|STATIC_MOMENT_KIND\|MOMENT_FAMILY" src/domain/services` och lägg in typen i varje register den faktiskt ska synas i (med en kommentar som pekar på domen, samma mönster som redan finns i alla fyra tabellerna). Skriv INTEGRATIONSTESTET (hela vägen `redaktoren()` → `agendaForSurface` → konsument) före enhetstesterna på producent/konsument var för sig — ett enhetstest på konsumenten ensam (`selectMissedTargetCallback` kallad direkt med en handbyggd post) hade varit grönt även utan registreringen, och hade dolt precis det grindfel som fanns.
