# SPEC: Pep-talk-system

**Datum:** 9 april 2026
**Typ:** Ny feature — Code
**Berör:** Ny service + DashboardScreen CTA-sektion

---

## Vad

En kursiv tränarcitat-rad visas ovanför CTA-knappen på Dashboard, under DiamondDivider. Citatet baseras på matchresultat, tabellposition och streak. Byter varje omgång.

## Var

Ny fil: `src/domain/services/pepTalkService.ts`
Ändrad fil: `src/presentation/screens/DashboardScreen.tsx`
...
## DashboardScreen.tsx — ändring

Import (lägg till bland övriga imports):
```typescript
import { getPepTalk } from '../../domain/services/pepTalkService'
```

I CTA-sektionen, EFTER `<DiamondDivider />`, FÖRE `{canSimulateRemaining && (`:
...
**OBS:** Ta INTE bort CareerStatsCard eller annan befintlig kod. Pep-talk läggs till som en ny rad, inget ersätts.
...
Pep-talk ska synas under DiamondDivider på dashboard efter omgång 1. Ska byta text varje omgång baserat på `roundNumber`. Ska vara kursiv Georgia, centrerad, fontSize 11, text-muted.
