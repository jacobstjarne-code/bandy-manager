...
**Byggt:**
- **Sorteringen (den verkliga fixen):** ny `getSelectionScore(player) = currentAbility × playerModifier(player)` (squadEvaluator.ts) — samma multiplikativa mönster `evaluateSquad()` redan använder, CA som positions-agnostisk proxy. `spelklarhet()` RADERAD, inte lämnad som en andra sanning (se docs/archive/historiska-statuskallor/BACKLOG.md "Två läsare, en sanning", raden nu STÄNGD).
