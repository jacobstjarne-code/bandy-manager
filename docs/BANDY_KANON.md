...
**Formationen bär höjdläget — och bara det.** `press`-axeln är borta som reglage och som fält på `Tactic`; `heightMode` härleds ur formationen (541 = låg, 532 = mellan, 523 = hög) och `getTacticModifiers` läser den med exakt de tal press hade. 5-2-3 kostar dessutom kondition per omgång ("väldigt kraftödande, kortare perioder") — konservativ konstant, magnitud ur kalibreringsrundan C2. De fyra 5-3-2-formerna har ingen egen multiplikator — de verkar genom slot-kartan, positionspassningen och kemin (`532_hogahalvor` delar slot-id:n men lyfter halvornas y). Rollen skapar skillnaden (Andersson 2022, van den Tillaar 2023). Fotbollsswitchen i `tacticModifiers` (fyra bak = tryggare) är borttagen: ingen källa stödde den. Sju axlar kvar på taktikskärmen, rubrik "Uppställning".

**Historik:** fram till 2026-09-04 hade spelet sex formationer med varierande försvarslinje, bandy-anatomiska slots och citat (`TEXT_REVIEW_formations_2026-04-20.md`, arkiveras) men en fotbollsformad motoreffekt (2-3-2-3 offense +0.05/defense −0.08, 4-3-3/4-2-4 −0.03/+0.05) som kanon felaktigt beskrev som "påverkar INTE" tills 2026-09-04. Taggregeln från 2026-04-20 (anatomi, inte effekter) står i V2. `TacticPress`-enumet är kvar i koden som superseded (26 testfiler konstruerar det) — medvetet.
...
1. **Formationsaxeln — DÖMD OCH BYGGD 2026-09-04.** Väg 1, sex uppställningar med femman bak, formationen bär höjdläget, `press` borta, fotbollsswitchen borta. Commit 18ff34e3. Kvar: 5-2-3:s konditionskostnad kalibreras i C2; tre scripts utanför tsconfig (data-warehouse generate/validate, press-low-vs-medium) refererar gamla formationer och måste uppdateras före nästa mätning.
