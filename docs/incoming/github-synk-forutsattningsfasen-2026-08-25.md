...
### Batch-av-tre + skuggkanon (2026-08-20)
- "Batch-av-tre och skuggkanon - D1p4 A14.dc.html". BATCH (D1 punkt 4): normal-viktens saknade läge. Rek 1b = skymtad stapel (två slivrar bakom aktivt kort) + räknare "Två till efter den här"/"En till" (låst copy). Övergång: besvarat sjunker/tonar ut nedåt, nästa reser sig (fadeInUpScale) — "beta av" ej "bläddra", aldrig sidled. Plats: där Primary ligger (batchen ÄR omgångens primära krav); QueueRail bär fortsatt ambient. Pivotal mitt i → bryter stapeln, EventOverlay ensamt, återupptas. När INTE batcha: gräns = delad trigger, ej antal. Samma händelse → batch (≤3); orelaterade → separata kort. SKUGGKANON (Å14): 3 elevationsnivåer × 2 lägen — rest(=nuv card)/raised(=nuv menu)/modal(NY 0 12px 40px/.28). Mörkt läge: rim-ljus (inset vit) + djupare drop, modal-dark = kopparrim. Accentskuggor (-primary/-danger) står UTANFÖR skalan (signatur, ej elevation), -btn behålls. Modalytor: EventOverlay+Substitution=modal(ljus/centrerad), TacticChangeModal=modal-dark(dockad, Å10) — samma nivå olika läge. Ställen med 0 20px 40px SaaS-skugga mappar mot inget → flaggas, tvingas ej in.
