...
| 19 | ~~**D1** punkt 1–3: Å7-padding, ambient-regeln, konsekvensmarkören~~ KLAR (`97d26cfd`/`4e347971`/`9ae907bc`) — punkt 4 "därför nu"-mekanism KLAR (`7e1205ba`), ej aktiverad (väntar på Opus/Jacobs klassificering av ~16 befintliga critical-events). Batch-av-tre väntar på mock | `DOM_D1_EVENTVIKTNING_2026-08-19.md` |
...
| 36 | **Å12–15** nav-dokdrift, emoji-rester, egna skuggor — Å12 `KLAR`, Å14 delvis `KLAR` (`2f403ff0`), se not. Å13/Å15 `EJ` |
...
| 6.4 | Kontrastgrind, en-primär-grind, träffytegrind, matchtypsmatris-grind, datarobusthetsgrind (åtgärdslistans post 17–21) | `PÅGÅR` — post 17, 18, 20 `KLAR`. Post 21 `DELVIS KLAR` (DecisionCard-dubbelpadding, post 7), se noter nedan. Post 21:s andra två deldelar (datakortens robusthet mot långa namn/svensk pos/skada/tomt pris-kort, post 5/11) `EJ` — flaggat, kräver edge-case-fixturer, inte byggt än |
...
**6.4, post 21 — DELVIS KLAR, tre skilda deldelar, en byggd.** Post 21 (`Datakortens robusthet + DecisionCard-kontextmatris + takregel-scenmatrisen`) buntar tre av åtgärdslistans ursprungsfynd (post 3, post 5, post 11 + del av post 7). Läget per deldel:
...
| Å12–15 | Skuld: nav-dokdrift (6 flikar dokumenterat, 7 i bruk), emoji-svepets rester, egna skuggor mot skuggkanon, bundle | Å12 `KLAR (2f403ff0)` — `design-system/README.md` uppdaterad, Bygget saknades helt, Transfers beskrevs som fast istf villkorad. Å14 delvis `KLAR (2f403ff0)` — en exakt token-träff (`GameHeader.tsx`), resten (~25 ställen) väntar på ett `--shadow-modal`-kanonbeslut (Design), inte mekanisk. Å13 (emoji-svepets rester) + Å15 (bundle, redan synlig i build-output som >500kB-varning, kräver code-splitting) `EJ` |
...
| ID | Post | Status |
|---|---|---|
| D1 | Eventköns viktning: ambient / normal / pivotal, plus konsekvensnivå på val (neutral, positiv, kostsam, irreversibel). Inkl. Å7:s inline-rytm och Å8:s taktikviktning. Svåraste frågan: hur pivotal får väga mer **utan** att bli en fjärde ceremoni | `DELVIS KLAR` — se `docs/DOM_D1_EVENTVIKTNING_2026-08-19.md`. Punkt 1 (Å7, dubbelpadding) redan `KLAR (97d26cfd)` innan detta pass, orört. Punkt 2 (Ambient-regeln) `KLAR (4e347971)` — `isAmbientEvent`/`getEventRenderTarget` i `eventQueueService.ts`, ny `AmbientEventRow.tsx`, wirad i `PortalEventSlot.tsx`/`GameShell.tsx`/`EventPrimary.tsx`; fann och fixade under vägen en verklig softlock (kritiskt event utan val gav en fullskärmsmodal utan knappar) samt en krasch i `gameFlowActions.ts`s `simulateRemainingStep`. Punkt 3 (konsekvensmarkören) `KLAR (9ae907bc)` — `EventChoice.consequenceLevel`/`costLabel`/`irreversible` + `getConsequenceLines()` i `GameEvent.ts`, rendering i `DecisionChoices.tsx`; grep-verifierat 0 träffar på `--danger`/⚠ i den nya koden. Renderingsstöd byggt men opt-in — inga befintliga events sätter de nya fälten än. Punkt 4, "därför nu"-raden — MEKANISM `KLAR (7e1205ba)`: `GameEvent.deadlineLabel`/`whyNowPerson`/`wholeEventIrreversible`/`seasonDefining` + `getWhyNowLine()` (låst copy, prioritetsordning) + `getEffectivePriority()` i `eventQueueService.ts` + rendering i `DecisionCard`/`EventOverlay`. **Nedgraderingen är INTE aktiv** — `getEffectivePriority` läses inte av routing/sortering än, eftersom ingen av de ~16 befintliga `critical`-konstruktionsställena (mecenatEvent×11, criticalEconomy×3, playerUnhappy, economicStress) sätter whyNow-data. Vilken av de fyra grunderna som driver brådskan per event är en innehållsbedömning (Opus), inte kod-gissning — Jacob/Opus behöver gå igenom de ~16 ställena innan self-enforcement kan slås på. Punkt 4, batch-av-tre — INTE byggd, ingen stapel-UI finns alls i kodbasen, ny visuell/interaktiv form utan mock i `docs/mockups/` → mock-driven design-principen gäller, väntar på Opus-mock. 25 nya tester totalt (`eventRenderRouting.test.ts` + `consequenceMarker.test.ts` + `gameEventWhyNow.test.ts` + `eventQueueEffectivePriority.test.ts`), stash-verifierade. |
