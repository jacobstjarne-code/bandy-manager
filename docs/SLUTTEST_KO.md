...
**Sidofynd från samma sweep, RAPPORTERAT men INTE fixat:** `generateVarselEvent`s "offer_pro"-val sätter hela truppens lön till hårdkodad 0 kr (`value: 0` på `makeFullTimePro`-subeffekten, `salary: sub.value ?? p.salary` — 0 är inte nullish) trots att subtitlen lovar "höjd lönekostnad". Ekonomisk bugg, inte en fältnamnsmiss — annan klass än det Jacob beställde grepet för, väntar egen dom.
