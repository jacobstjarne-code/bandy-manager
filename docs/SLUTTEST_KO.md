...
| 40b | ~~**Skutskär-auditens fixordning, punkt 1 och 3–7**~~ KLAR 2026-08-22 (Jacobs order, punkt 2/styrelsemodellen redan gjord innan detta pass). Se `docs/SKUTSKAR_AUDIT_FIXORDNING_2026-08-22.md` för full spårning. Byggt: High 4 pressminnet, High 6 skadekortet, Medium 6 domarcitatet, High 3 playoff-barriären, Medium 5 säsongsslutsbarriär + Medium 1 sponsordedupe + Low 1 next-action, Medium 4 critical per instans + Medium 3 Form-etiketten, Medium 7 deep-link-rehydrering + Low 2 PWA-versionsskifte, **High 2 utmattningen** (Jacobs dom 2026-08-22: sorteringen bytt till `getSelectionScore` (currentAbility×playerModifier), `spelklarhet()` raderad, en dold tredje kopia av urvalslogiken (`useLineupEditor.ts`) konsoliderad in i delad `pickBestEleven()`, `SPELKLARHET_FITNESS_FLOOR=22` med tunn-trupp-fallback, Granska-copy-kopplingen hårdstyrd till `bad` under golvet). High 5 (BatchStack) — INTE i Jacobs byggordning, medvetet skippad. Medium 2 (mecenatpoolen) beställd i samma dom, separat pass. Domarcitatets `loss`-hink har `'[Opus]'`-platshållare, väntar text |
