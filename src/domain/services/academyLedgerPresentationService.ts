...
export function academyEconomyYearbookLine(game: SaveGame, season = game.currentSeason): string {
  ...
  return `Akademin: investering ${Math.round(startCost / 1_000)} tkr · drift ${Math.round(operatingCost / 1_000)} tkr. Gav ${promotedCount} uppflyttade och ${developmentGain} i utveckling.`
}
