...
export function resolveFreeKick(
  ...
  if (choice === 'shoot') {
    ...
    if (rand() < goalChance) {
      return { type: 'goal', description: `${kicker.firstName} ${kicker.lastName} skruvar bollen förbi muren och in! Frislagsmål!` }
    }
  ...
  if (choice === 'chipPass') {
    ...
    if (rand() < passSuccess) {
      ...
      if (rand() < goalChance) {
        return { type: 'goal', description: `Lyftare över muren — en medspelare möter och sätter in!` }
      }
      return { type: 'corner_won', description: `Bollen lyfts bakom muren — avslutet blockeras och det blir hörna.` }
    }
  ...
  if (rand() < goalChance) {
    return { type: 'goal', description: `Kort frislag, upplägg — och ett välplacerat avslut letar sig in!` }
  }
