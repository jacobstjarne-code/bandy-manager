...
describe('applyRoundDevelopment — mentor (PC-6)', () => {
  const MANAGED = 'club_test'
  const noRatings: Record<string, number> = {}

  function youngProspect(id: string): Player {
    // Ung spelare med stort gap mot PA — tydlig positiv utveckling, rum för mentor-bonus
    return makePlayer({ id, age: 18, currentAbility: 40, potentialAbility: 85, developmentRate: 80 })
  }

  it('a mentored young player gains more CA than an identical un-mentored player', () => {
    const mentored = youngProspect('mentored')
    const control = youngProspect('control')
    const played = new Set<string>()        // ej spelat — isolerar mentor-effekten från matchbonus
    const starters = new Set<string>()

    const mentorEntries = [{ action: 'mentor', expiresRound: 100, mentoredPlayerId: 'mentored' }]

    let mentoredCur = [mentored]
    let controlCur = [control]
    for (let round = 1; round <= 10; round++) {
      mentoredCur = applyRoundDevelopment(mentoredCur, MANAGED, 'physical', 'normal', played, starters, noRatings, mentorEntries, round)
      controlCur = applyRoundDevelopment(controlCur, MANAGED, 'physical', 'normal', played, starters, noRatings, [], round)
    }

    const mentoredGain = mentoredCur[0].currentAbility - mentored.currentAbility
    const controlGain = controlCur[0].currentAbility - control.currentAbility
    expect(mentoredGain).toBeGreaterThan(controlGain)
  })

  it('mentor bonus stops once the entry has expired', () => {
    const mentored = youngProspect('mentored')
    const played = new Set<string>()
    const starters = new Set<string>()
    // Entry löper ut efter omgång 3
    const mentorEntries = [{ action: 'mentor', expiresRound: 3, mentoredPlayerId: 'mentored' }]

    // CA efter att fönstret stängts, jämfört med kontroll utan mentor — ska vara lika från omg 3+
    let withCur = [mentored]
    let ctrlCur = [youngProspect('mentored')]  // samma id/start så utvecklingen är identisk
    for (let round = 4; round <= 8; round++) {
      withCur = applyRoundDevelopment(withCur, MANAGED, 'physical', 'normal', played, starters, noRatings, mentorEntries, round)
      ctrlCur = applyRoundDevelopment(ctrlCur, MANAGED, 'physical', 'normal', played, starters, noRatings, [], round)
    }
    // Från omg 4 (efter expiry) ska mentor-grenen inte ge något extra — identisk utveckling
    expect(withCur[0].currentAbility).toBeCloseTo(ctrlCur[0].currentAbility, 5)
  })

  it('mentor bonus never pushes CA past potentialAbility', () => {
    // Spelare nära sitt tak — bonus ska klampas av PA
    const nearCap = makePlayer({ id: 'nearcap', age: 19, currentAbility: 84, potentialAbility: 85, developmentRate: 90 })
    const played = new Set<string>(['nearcap'])
    const starters = new Set<string>(['nearcap'])
    const ratings = { nearcap: 8.5 }
    const mentorEntries = [{ action: 'mentor', expiresRound: 100, mentoredPlayerId: 'nearcap' }]

    let cur = [nearCap]
    for (let round = 1; round <= 20; round++) {
      cur = applyRoundDevelopment(cur, MANAGED, 'physical', 'normal', played, starters, ratings, mentorEntries, round)
    }
    expect(cur[0].currentAbility).toBeLessThanOrEqual(nearCap.potentialAbility)
  })

  it('mentor entry with no mentoredPlayerId is a no-op', () => {
    const young = youngProspect('young')
    const played = new Set<string>()
    const starters = new Set<string>()
    // Mentor-åtgärd utan ung spelare att mentorera (mentoredPlayerId saknas)
    const mentorEntries = [{ action: 'mentor', expiresRound: 100, mentoredPlayerId: undefined }]

    let withCur = [young]
    let ctrlCur = [youngProspect('young')]
    for (let round = 1; round <= 10; round++) {
      withCur = applyRoundDevelopment(withCur, MANAGED, 'physical', 'normal', played, starters, noRatings, mentorEntries, round)
      ctrlCur = applyRoundDevelopment(ctrlCur, MANAGED, 'physical', 'normal', played, starters, noRatings, [], round)
    }
