  ...
  if (rand() < 0.40 && peakArcs.length > 0) {
    ...
    if (arcPlayer) {
      const arcQuestions: Partial<Record<import('../entities/Narrative').ArcType, string>> = {
        ...
        veteran_farewell: `Blir det här ${swedishGenitive(`${arcPlayer.firstName} ${arcPlayer.lastName}`)} sista säsong?`,
  ...
  if (rand() < 0.30 && storylines.length > 0) {
    ...
    if (ctx.won && underdogStory && storylineBudgetOk(underdogStory)) {
      ...
      if (rescuePlayer && rescueScorerMatch) {
        question = { text: `Berätta om ${swedishGenitive(`${rescuePlayer.firstName} ${rescuePlayer.lastName}`)} resa tillbaka.`, preferIds: ['tp_liv1', 'tp_liv4', 'tp_liv2'] }
