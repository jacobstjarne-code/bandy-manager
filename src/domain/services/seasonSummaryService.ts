...
// THE_BOMB 2.2 — "Årets match": picks the single most dramatic managed fixture
...
export function pickSeasonHighlight(game: SaveGame): SeasonHighlight | null {
  const managedClubId = game.managedClubId
  ...
  if (clubFixtures.length === 0) return null

  let bestFixture: typeof clubFixtures[number] | null = null
  let bestScore = -1
  let bestIsHome = false
  let bestMyScore = 0
  let bestTheirScore = 0
  ...
  for (const f of clubFixtures) {
    ...
    let score = 0
    score += Math.max(0, 10 - Math.abs(margin) * 2)
    score += f.events.filter(e => e.type === MatchEventType.Goal && (e.minute ?? 0) >= 80).length * 15
    if (isDerby) score += 25
    if (margin > 0) {
      const firstGoal = f.events.find(e => e.type === MatchEventType.Goal)
      if (firstGoal && firstGoal.clubId !== managedClubId) score += 20
    }
    if (f.roundNumber >= 18) score += 10
    if (game.captainPlayerId && f.events.some(e => e.type === MatchEventType.Goal && e.playerId === game.captainPlayerId && e.clubId === managedClubId)) score += 10
    if (f.events.some(e => {
      if (e.type !== MatchEventType.Goal || e.clubId !== managedClubId || !e.playerId) return false
      const p = game.players.find(pl => pl.id === e.playerId)
      return p?.promotedFromAcademy && p.age <= 22
    })) score += 10
    ...
    if (score > bestScore) {
      ...
      bestIsHome = isHome
      bestMyScore = myScore
      bestTheirScore = theirScore
    }
  }

  if (!bestFixture) return null

  const isHome = bestIsHome
  const myScore = bestMyScore
  const theirScore = bestTheirScore
  const oppId = isHome ? bestFixture.awayClubId : bestFixture.homeClubId
  const opp = game.clubs.find(c => c.id === oppId)
  const oppName = opp?.shortName ?? opp?.name ?? '?'
  const scoreStr = isHome ? `${myScore}–${theirScore}` : `${theirScore}–${myScore}`
  const margin = myScore - theirScore
  const lateGoals = bestFixture.events.filter(e => e.type === MatchEventType.Goal && (e.minute ?? 0) >= 80 && e.clubId === managedClubId)
  const rivalry = getRivalry(bestFixture.homeClubId, bestFixture.awayClubId)

  let description = ''
  if (rivalry && margin > 0) description = `En derbyseger att minnas. ${oppName} besegrades i ${rivalry.name}.`
  else if (lateGoals.length > 0) {
    const p = lateGoals[lateGoals.length - 1].playerId ? game.players.find(pl => pl.id === lateGoals[lateGoals.length - 1].playerId) : null
    description = p ? `${p.firstName} ${p.lastName} avgjorde i ${lateGoals[lateGoals.length - 1].minute ?? 80}:e minuten.` : `Sent avgörande mot ${oppName}.`
  } else if (margin > 0) {
    description = `En övertygande seger med ${margin} måls marginal mot ${oppName}.`
  } else if (margin < 0) {
    description = `En tuff förlust mot ${oppName} — en match som ger lärdomar.`
  } else {
    description = `En jämn batalj mot ${oppName}. Rättvisa poäng delades.`
  }

  const leader = game.supporterGroup?.leader.name
  if (leader && margin > 0) description += ` — "${oppName} fick se vad vi går för." — ${leader}`

  return {
    headline: margin > 0 ? `🏒 SÄSONGENS MATCH` : `📋 MATCHDAGEN ATT MINNAS`,
    score: scoreStr,
    round: bestFixture.roundNumber,
    opponent: oppName,
    description,
  }
}
