...
describe('getContentContractEntry', () => {
  ...
  it('låser patronEvent till säsongsdeduplicering, gemensam relationseffekt och verklig bonus', () => {
    ...
    expect(entry?.cooldownSeasons).toBeUndefined()
    expect(entry?.trigger).toContain('exakt en seedad save+säsong-rullning')
    expect(entry?.trigger).toContain('från 1 % till 22 % per säsong')
    expect(entry?.trigger).toContain('inget CS-läge är en absolut emergence-vägg')
