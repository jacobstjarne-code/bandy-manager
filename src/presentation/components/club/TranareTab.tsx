...
export function TranareTab({ game }: Props) {
  ...
  const topRivalries = [...profile.coachRivalries]
    .filter(r => r.clubId !== game.managedClubId && h2hTotal(r) > 0)
  ...
  const nemesis = deriveCoachNemesis(
    profile.coachRivalries.filter(rivalry => rivalry.clubId !== game.managedClubId),
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {topRivalries.map(rivalry => {
              ...
              return (
                <div key={rivalry.clubId} style={{ borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{club.name}</span>
      ...
      {nemesis && nemesisClub && (
        <div className="card-sharp" style={{ padding: '12px 14px', borderLeft: '3px solid color-mix(in srgb, var(--danger) 40%, transparent)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{nemesisClub.name}</span>
            <span style={{ fontSize: 10, color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>
