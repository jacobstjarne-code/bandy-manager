...
export function handlePlayoffStart(game: SaveGame, _seed?: number): AdvanceResult {
  ...
  const completedFixtures = game.fixtures.filter(f => f.status === FixtureStatus.Completed && !f.isCup && !f.isKnockout)
  // 4.1 (docs/archive/historiska-statuskallor/SLUTTEST_KO.md, 2026-08-17): pointDeductions saknades här — en klubb
