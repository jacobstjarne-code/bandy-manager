...
describe('createNewGame', () => {
  ...
  it('managerName and managedClubId are set correctly', () => {
    const game = createNewGame({ managerName: 'Jacob Stjärne', clubId: 'club_sirius', season: 2025, seed: 42 })
    expect(game.managerName).toBe('Jacob Stjärne')
    expect(game.managedClubId).toBe('club_sirius')
  })
  ...
  it('same seed produces same fixture schedule (deterministic)', () => {
    const game1 = createNewGame({ managerName: 'Jacob', clubId: 'club_sandviken', season: 2025, seed: 99 })
    const game2 = createNewGame({ managerName: 'Jacob', clubId: 'club_sandviken', season: 2025, seed: 99 })
