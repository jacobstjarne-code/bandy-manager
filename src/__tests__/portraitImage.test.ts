...
describe('portrait arketyp-wiring', () => {
  ...
  it('är deterministiskt per spelare och väljer bara ur respektive kuraterad filuppsättning', () => {
    ...
    expect(a).toBe(b)
    expect(a).toMatch(/^\/assets\/portraits\/portrait_vet_(?:[1-5]|[7-9]|1[0-6])\.png\?v=7$/)
    ...
    expect(getPortraitImagePath('player_42', 29)).toMatch(/^\/assets\/portraits\/portrait_exp_(?:[1-9]|[12]\d|30)\.png\?v=7$/)
  ...
  it('alla fyra åldersfack har en kuraterad bildväg', () => {
    ...
    expect(getPortraitImagePath('player_7', 29)).toMatch(/portrait_exp_(?:[1-9]|[12]\d|30)\.png\?v=7$/)
    expect(getPortraitImagePath('player_7', 35)).toMatch(/portrait_vet_(?:[1-5]|[7-9]|1[0-6])\.png\?v=7$/)
  })
})
